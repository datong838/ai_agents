...
> **版本**：v1.58 · 2026-08-05
> **状态**：🟡 TI-4 全域、TI-5 最终 GREEN；下一门 TI-6 迁移收口与生产前门
...
- **TI-6-0R 只读重基线**：重新运行 PostgreSQL、对象、向量、缓存、队列、Scheduler、Process Memory precheck，输出到 Git 外临时目录；不得先改数据再制造 GREEN。当前重扫结果为 95/95 已登记表扫描成功、栖月汇业务 0，但静态注册状态滞后、6 张维护 ledger/quarantine 表未登记、未知对象/向量及未配置后端仍使 `isProvenEmpty=false`。
...
评审通过后的 **TI-0** 已全部完成；TI-1 E1～E3 非生产工程、TI-PRE-1、TI-2 E1～E7、TI-3 E1～E7 与 TI-4 全域已完成。TI-1 E4 仍因 59 条逻辑隔离记录 BLOCKED；按连续执行授权，下一步执行 TI-5 AIP/Analytics/模型目录与非数据库资源隔离，但不得开始具体商城 Connector。
