...
> **版本**：v1.2 · 2026-08-04
> **状态**：🟡 全域审计已完成，统一方案待评审确认；未进入编码和数据迁移
...
| 空数据定义 | 不是“某几个列表为空”，而是租户资源注册表中所有 tenant-owned 资源计数均为 0，且对象存储、向量、缓存和任务队列无该租户前缀数据 |
...
| ADR-TI-008 | `twa_*` 与 `meta_*` 组织目录单一真源选择 | 建议 `twa_*`，评审确认 |
| ADR-TI-009 | Ontology 系统模板 + 租户 overlay | 待 TI-5 细案冻结 |
...
- [ ] 当前 86 张 public 表均被机器可读注册表分类；新增表未注册时 CI 失败。
...
- [ ] 所有非公开业务路由无 Token 返回 401；无鉴权 Router/OpenAPI 静态扫描为 0。
...
1. 是否同意保留 `dev-org` 技术 ID，仅把显示名改为“测试组织”。
2. 是否同意栖月汇初始“已安装应用 = 0”，四应用只出现在可安装模板区。
3. 是否同意删除 Module 默认保留订单等业务事实，数据清除使用独立动作。
4. 是否同意 `twa_*` 作为组织/工作区权威真源，`meta_*` 逐步收敛为投影或下线。
5. 是否同意先完成 TI-0/TI-1 公共底座，再由四 Worker 并行执行 TI-2～TI-5。
6. 是否同意“千人千面”采用“模板 + 组织 Profile + 工作区 InstanceOverlay + 有限用户偏好”的版本化继承模型，禁止客户源码分叉。
7. 是否同意 TI-0C 先将 Data OS Phase 5/6、Pipeline Builder 和 Module 子资源无鉴权路由失败关闭，再执行各域存储迁移。
8. 是否同意将当前 InstanceOverlay 视为 synthetic 合约基础，生产 Store/API/Policy/Evidence 由 TI-2 按既有资产安装架构实现。

评审通过后，第一波只执行 **TI-0：全量资源注册表、数据归属 precheck、组织改名迁移计划和栖月汇空数据基线**，不立即迁移或删除业务数据。
