...
> **版本**：v1.58 · 2026-08-05
> **状态**：🟡 TI-4/TI-5 最终 GREEN、TI-6-1 GREEN；下一门 TI-6-2 剩余结构阻断
...
#### 13.11 TI-PRE-1 共享 PostgreSQL 测试隔离修复（2026-08-04）

E3 完成后的扩大回归暴露三项既有测试债务：TWA 组织进入、邀请接受和工作区删除用例使用固定 `org-a/org-b/prj-del-*` 标识，同时测试进程内 Store 会在应用启动时从共享 PostgreSQL 重新加载。固定夹具、跨用例残留和启动重载相互叠加，使单测结果依赖执行顺序，并在共享开发库产生新的测试成员/工作区行。该问题不归因于 E3，但若不先修复，后续 TI-2～TI-6 的跨租户证据不可复现。

本波固定边界：

1. 只修改测试夹具、测试辅助能力及对应文档；不修改客户/生产数据，不清理 `org-org`，不把未知历史行认领给任何组织。
2. 测试组织、组织、工作区和 subject 使用每用例唯一命名空间；测试结束只按本用例精确登记的键逆序清理，禁止 `LIKE 'org-%'`、全表 `DELETE/TRUNCATE` 或固定共享 ID 清理。
3. 进程内 Store 与 PostgreSQL 必须采用同一测试模式。普通单元测试保持 `AOS_TWA_STORE=memory`；需要验证 PostgreSQL 的用例使用独立临时数据库，不得连接共享 `aos_meta` 执行可变测试。
4. 若当前测试框架暂不能提供每用例临时数据库，则本波先将 TWA API 回归完全固定为 memory，并增加“共享数据库关键表前后 hash/计数不变”守卫；PG 持久化专项测试另走独立库。
5. 修复退出门：三项失败用例单独和组合均 GREEN；authz/TWA 扩大回归 GREEN；共享库 `meta_org/meta_workspace/meta_membership/authz_tuple` 前后快照一致；不得通过删除客户数据或忽略断言获得 GREEN。

TI-PRE-1 是测试证据可信度门，不改变 TI-1～TI-6 架构、迁移顺序或 E3 的 59 条 QUARANTINE 结论。完成后才允许把 TI-2 的 APP-01～APP-15 结果作为租户隔离证据。

完成结论：代码 `0ae62bc` 已将普通 pytest 会话切换到自动创建、迁移并销毁的临时 PostgreSQL；memory TWA 不再触碰共享 `meta_*`。原失败组合 17 项、authz/TWA 扩大回归 99 项和租户隔离 71 项全部 GREEN；共享 `meta_org/meta_workspace/meta_membership/authz_tuple` 前后摘要一致，临时数据库残留为 0。详细证据见 `evidence/tenant-isolation/2026-08-04-TI-PRE-1共享PostgreSQL测试隔离证据.md`。
...
首个执行门详见 `228-TI-2-E1-Module实例键与Overlay基础扩展实施方案.md`；E1 只做 nullable/NOT VALID/空表 Expand，不回填或切换运行路径。

TI-2 E1 已由代码 `e95ae45` 完成：95 张 PostgreSQL 表/102 个总资源注册，Module nullable 实例键、3 张空的千面 revision 表和 11 个 NOT VALID FK 已落库；644 行既有数据守恒，栖月汇 Module 资源仍为 0。E2 前不得把 E1 解读为运行时隔离完成。

TI-2 E2-A/E2-B 已由代码 `b7cfb34`/`5df9b5e` 完成：Canvas、Widget、Variable、Query、Event、Interface 的当前运行路径显式接收 TenantScope；读、改、删同时约束组织、工作区、Module 与资源 ID，遗留全局主键冲突统一失败关闭。当前仍不支持两个租户复用相同遗留 module_id；该能力必须等 E3～E7 的 module_pk 回填、读切换和 Contract 后才能宣告完成。

TI-2 E2-C 已由代码 `87d6ef5` 完成：Deployment 部署/目标回滚、Theme 全 CRUD、Widget Catalog 目录读写均显式接收 TenantScope；跨租户目标回滚返回不存在，Theme/Widget 遗留全局 ID 冲突失败关闭。Platform Template 与租户目录的最终拆分仍属于后续 E2-D/E3～E7，不把当前租户化目录冒充平台共享模板。

TI-2 E2-D 已由代码 `472728e` 完成：Module aggregate 全业务方法以 TenantScope 为首参，`_DEFAULT_ORG/_DEFAULT_PROJECT` 从 10 个 E2 运行时 Store 清零并由静态测试守卫。APP-01/02/03 仅完成现有全局 ID 约束下的基础空态与单实例隔离；APP-04/05 仍是明确未完成门，不得用“跨租户失败关闭”替代“同 ID 可共存/可卸载”。

TI-2 E3 已由代码 `8e57c8a` 与共享非生产最终 batch 完成：160 个父 Module、462 个精确关联子记录写入确定性 module_pk；两条孤儿 Event 保持 NULL 并写入 quarantine；恢复库和共享库 drill 均完成 apply/verify/rollback 622 项，新写持续性门已补齐。E3 不改旧 PK、不切读，APP-04/05 仍未完成。

TI-2 E4 已由代码 `129d438` 完成：共享非生产库 11 个 Module 身份 FK 在逐项零违规 precheck 后完成 Validate，并通过 upgrade/downgrade/upgrade 可逆演练；最终 11/11 validated、行数与身份计数守恒、栖月汇 Module 0、RLS 0。E4 只收紧既有关系完整性，未切读；下一门为 E5 Read Switch，APP-04/05 仍未完成。

TI-2 E5 已由代码 `8aee9f6` 完成：父 Module 以稳定 module_pk 读取并影子核对 scoped legacy 身份，7 张直属子表切换为 `module_pk + TenantScope` 读改删；共享库 160 个父对读一致、子错配 0、两条孤儿 Event 新读不可见。API `moduleId` 不变；下一门为 E6 RLS，APP-04/05 仍未完成。

TI-2 E6 已由代码 `846b49a` 完成：应用 scoped transaction 降权为无登录/无超级权限/无 BYPASSRLS 的 `aos_runtime`，11 张 Module 实例表 ENABLE+FORCE RLS 并具备 USING/WITH CHECK；无 GUC可见 0，跨 scope 与伪造写失败关闭。共享库可逆演练和 134 项累计测试 GREEN；下一门为 E7 Contract，APP-04/05 仍未完成。

TI-2 E7 已由代码 `4004170` 完成：8 张 Module 父子活跃表主键收口为 TenantScope 复合键，module_pk 非空；2 条孤儿 Event 原样迁入 runtime 无权访问的维护隔离表；GET ETag 与 admin/owner + If-Match 软卸载已落地。APP-04 同 ID 双租户共存和 APP-05 当前 scope 卸载/业务对象守恒均 GREEN，真实升降级往返与 272 项累计回归通过。TI-2 E1～E7 最终 GREEN，下一执行域为 TI-3。
...
TI-3 E1 已由代码 `7f7d48a` 完成：5 张原 NO_TENANT 表新增 nullable org/project，9 张对象运行资源建立 NOT VALID workspace FK，1,030 行守恒，Ontology 三张平台模板不变；真实升降级往返和 100 项 Tenant Isolation 回归 GREEN。下一门为 E2 显式 TenantScope/新写双写，历史归属仍未回填。

TI-3 E2-A 已由代码 `f5f0b7c`/`5cdafea` 完成：Graph/Funnel/Branch/Overlay 新写与 Branch merge mutation 显式携带 TenantScope；遗留全局键跨 scope 重用失败关闭，不覆盖原记录。185 项累计回归 GREEN；下一门 E2-B Draft/Wiki/Lifecycle。

TI-3 E2-B 已由代码 `561148f` 完成：Draft approval 的 Object/Wiki/Draft mutation、Lifecycle/TTL/Insight Store 与 CLI 显式携带 TenantScope；跨 scope Wiki/Object 遗留全局键失败关闭，137 passed、17 skipped，零失败。`decision_lineage` 历史归属继续留给 TI-5；下一门 E2-C Aggregate/静态门。

TI-3 E2-C 与 E2 总收口已由代码 `5e58768` 完成：Object/Branch/Object Set/Tool/Marking/Graph/Funnel/Analytics 读链及健康聚合显式 TenantScope，Branch Store scope 参数强制化，请求期 DDL 移除，目标 SQL 静态门 GREEN；144 passed、40 skipped，零失败。下一门 TI-3 E3 历史归属与隔离。

TI-3 E3 已由代码 `21f8803` 完成：1,030 行逐记录决策为 NO_ACTION=993、QUARANTINE=37、ASSIGN/BLOCKED=0；恢复库与共享最终四角色流程 GREEN，业务表 DML=0。37 条未知归属继续运行时不可见；下一门 E4 FK Validate。

TI-3 E4 已由代码 `ec6fff4`/`c268062` 完成：9 个 Object Runtime workspace FK 在非 NULL orphan=0 后 Validate；真实开发库完成 upgrade/downgrade/upgrade，约束状态 9/9→0/9→9/9，1,030 行与 37 条 NULL quarantine 守恒，业务 DML=0；Tenant Isolation 111 passed、7 skipped。最终 Alembic 为 `228ti3e4validate`，下一门 E5 Read Switch。

TI-3 E5 已由代码 `3d53105` 完成：全包 Object Runtime SELECT 无 scope 命中清零，vector autoSample、demo story/seed/clear 均显式 TenantScope，其他工作区对象和共享模板守恒；剩余 4 个无 scope Connector writer 精确延后 TI-4。共享库 1,030/37/0 守恒，下一门 E6 RLS。

TI-3 E6 已由代码 `8dfa626` 完成：9 张 Object Runtime 表复用 `aos_runtime`，全部 ENABLE/FORCE RLS 与双 GUC USING/WITH CHECK；无 GUC零可见、跨 scope/伪造写失败关闭。真实升降级与 1,030/37/0 守恒 GREEN；4 个 Connector owner writer 仍为 TI-4 阻断，下一门 E7 Contract。

TI-3 E7 已由代码 `fd2a124` 完成：9 表 TenantScope NOT NULL 与复合主键、overlay→branch scoped 复合外键全部生效；37 条未知归属 Object Runtime 历史行原样进入不可变维护隔离表，活跃 993 + 隔离 37 = 1,030。Object/Wiki/Graph/Branch/Funnel/Lifecycle/Draft 现有写路径已映射 scoped conflict target，同 ID 双租户与跨 scope FK 负向门 GREEN。共享库可逆往返和 160 项累计回归通过；4 个 Connector owner writer 继续 TI-4 失败关闭。TI-3 E1～E7 最终 GREEN，下一执行域 TI-4。
...
TI-4 C1 已 GREEN：代码 `ec0d7e7` 为 5 张空电商 Connector 真源表新增到 `twa_workspace(org_id, project_id)` 的 NOT VALID workspace alias FK，业务 DML=0；真实共享库升降级往返、专项 3 项和累计 126 项测试通过。5 表尚未启用 RLS，4 个 Object ingest writer 的 scope 丢失留给 C2，不连接具体商城。独立方案与证据见《228-TI-4-C1-电商Connector工作区外键扩展实施方案》和 `evidence/tenant-isolation/2026-08-04-TI-4-C1-电商Connector工作区外键扩展证据.md`。

TI-4 C2 已 GREEN：代码 `4ae0492` 复用 canonical `TenantScope`，把 Router Principal scope 传到 REST/File/MySQL/PostgreSQL/SQL Server 的 `obj_instance` transaction，写入 scope 列并切换 scoped conflict target；4 个 deferred writer 收口为 0，无 scope 直调失败关闭。共享库业务 DML=0，累计 132 项测试通过。C2 无 schema migration，不连接具体商城。独立方案与证据见《228-TI-4-C2-通用Connector显式TenantScope与对象写入实施方案》和 `evidence/tenant-isolation/2026-08-04-TI-4-C2-通用Connector显式TenantScope证据.md`。

TI-4 D1 已 GREEN：代码 `8e49b68` 为 5 表新增 10 个 nullable scope 列，并为 7 张 Data OS 表建立 NOT VALID workspace FK；共享库 297 行守恒、业务 DML=0、真实升降级往返和累计 135 项测试通过。Data OS 基表仍依赖既有 runtime bootstrap 的过渡风险已冻结，D7 Contract 前收归迁移。独立方案与证据见《228-TI-4-D1-DataOS租户列与工作区外键扩展实施方案》和 `evidence/tenant-isolation/2026-08-04-TI-4-D1-DataOS租户列与外键扩展证据.md`。

TI-4 D2 已按连续执行授权分为 D2-A/D2-B：先收口 Source/Pipeline/Dataset/History/Sync/Schedule 六类新写，再收口 Graph 与删除 mutation。所有 Store 显式 TenantScope，跨 scope 全局 ID 冲突失败关闭；不回填历史、不切全局读取。独立方案见《228-TI-4-D2-DataOS显式TenantScope写链实施方案》。

TI-4 D2 已 GREEN：D2-A 代码 `5155d9b` 完成六类 persist，D2-B 代码 `7a05bd8` 完成 Graph 与删除链 scoped mutation，并停止 boot 阶段无 scope 物理删数。共享库 297 行零改写，累计 155 passed / 8 skipped。当前 4 条记录具备完整 scope、293 条缺失 scope；下一门 D3 只建逐记录归属账本与逻辑隔离，禁止猜测回填。独立方案见《228-TI-4-D3-DataOS历史归属账本与逻辑隔离实施方案》。

TI-4 D3 已 GREEN：代码 `78e16a7` 对 297 条 Data OS 历史记录生成确定性 append-only 账本，4 条 `NO_ACTION`、293 条 `QUARANTINE`、`ASSIGN=0`，共享非生产 apply/verify 业务 DML=0。下一门 D4 只验证 7 个 workspace FK 的非 NULL 记录，不能把 FK Validate 表述为 NULL 历史已经归属。

TI-4 D4 已 GREEN：代码 `176bdf5` 完成 7/7 workspace FK Validate 与真实 D1→D4→D1→D4 往返；非 NULL orphan=0，297 行、293 NULL 和 D3 逻辑隔离守恒。下一门 D5 收口 `load_all/boot_data_os` 与 Data OS API 读取。

TI-4 D5 已 GREEN：代码 `2b81a1b` 将 `load_all` 与 Source/Sync/Dataset/History/Pipeline/Build/Schedule 读取切为严格 TenantScope，启动改为零全库装载的 lazy scoped cache；293 条 NULL 历史默认不可见。下一门 D6 RLS。

TI-4 D6 已 GREEN：代码 `3e9677f` 为 7 张 Data OS 表启用并强制 RLS，双 GUC `USING/WITH CHECK`、无 GUC 零可见、跨 scope 写失败关闭；共享库完成 D4→D6→D4→D6 可逆演练，297/293 与 D3 账本守恒。下一门 D7 将 293 条未知归属物理迁入不可变维护隔离区，并冻结 scoped 主键、父子 FK 与 Alembic 唯一建表真源。

TI-4 D7 已 GREEN：代码 `c890a7a` 将 293 条 D3 quarantine 原样迁入运行角色不可见的不可变维护隔离区，活跃 4 + 隔离 293 = 297；7 表 scoped PK/NOT NULL、6 条真实父子 FK、迁移唯一建表真源和 fresh database 门全部通过。Data OS D1～D7 最终 GREEN；下一门 C3 完成 5 张 Connector 真源表的 workspace FK Validate、RLS 与 runtime 降权 transaction。

TI-4 C3 已 GREEN：代码 `6feb1cb` 完成 5/5 电商真源表 workspace FK Validate、ENABLE/FORCE RLS 与双 GUC policy；SQLAlchemy 一致性事务、PostgreSQL OAuth CRUD 和按租户 due refresh 均已降权并显式绑定 TenantScope。共享库完成 D7→C3→D7→C3 可逆演练，5 表继续 0 行；209 passed / 11 skipped，五分支同 HEAD/tree。Data OS 与 Connector 已收口，但冻结执行组 `ti4-async-runtime` 尚未完成，下一门 A1。
...
#### TI-6 执行分波（2026-08-05 冻结）

- **TI-6-0R 只读重基线**：重新运行 PostgreSQL、对象、向量、缓存、队列、Scheduler、Process Memory precheck，输出到 Git 外临时目录；不得先改数据再制造 GREEN。首扫结果为 95/95 已登记表成功、栖月汇业务 0，同时发现 27 张静态状态滞后和 6 张维护表漏登记；TI-6-1 校准后重扫为 101/101、schema drift=0、栖月汇业务仍为 0。未知对象/向量、历史空 scope 与未配置外部后端继续显式阻断严格 `isProvenEmpty`。
- **TI-6-1 Registry/Contract 对账**：先把 27 张已完成迁移但注册表仍为旧状态的表更新为真实 `tenantColumns/primaryKey/STRONG_PK`；把 6 张维护 ledger/quarantine 表以维护/控制面语义登记并纳入唯一 execution group。新增 live schema drift 门，注册快照与真实 public schema 不一致即失败。只改机器真源与测试，不改业务数据。
- **TI-6-2 剩余结构阻断**：基于更新后的重扫只处理真实剩余的 tenant-owned NOT NULL、复合 PK/FK、RLS/role 绕过问题；历史归属证据不足的记录继续逻辑/物理维护隔离，不默认归栖月汇。每个结构动作单独 migration、临时库可逆演练和共享库备份门。
- **TI-6-3 空组织与应用实例门**：生成栖月汇全资源空数据报告；验证已安装应用为 0、测试组织四应用不泄露、同模板不同 Overlay、安装/卸载/删除只影响当前 scope，业务事实不随卸载删除。只使用 synthetic/测试实例。
- **TI-6-4 清理/删除/恢复演练**：执行资源注册表驱动的 dry-run impact plan、临时租户 apply/verify/rollback、备份恢复和幂等重试；不得删除栖月汇、测试组织或用户真实数据。未知历史、外部未配置后端和不可逆动作保留显式阻断。
- **TI-6-5 最终对账**：累计 Tenant Isolation、Schema/Resource/OpenAPI/route lint、全量收集、五分支同线、总计划/228 路线/上下文/evidence 一致。区分“本地开发接入准备 GREEN”和“生产外部后端/真实凭据部署条件”；前者完成后即在真实微商城 Connector 编码前暂停。

#### TI-6-4 清理/恢复演练详细执行门（2026-08-05 冻结）

1. **注册表是唯一资源范围**：只计划 `classification=TENANT_OWNED` 且 `baselineScope=BUSINESS_DATA` 的资源；平台模板、全局配置、组织/工作区根、成员关系和 TI 维护 ledger 不得进入业务数据清理动作。计划必须回显注册表版本、组织/工作区、每资源影响行数、处置策略、阻断原因与确定性 `planHash`。
2. **四类处置语义不混合**：普通当前态为 `DELETE_SCOPED_ROWS`；存在 `DELETE/TRUNCATE` 不可变防护的审计、修订和证据表为 `RETAIN_IMMUTABLE_HISTORY`；只有 `org_id` 的组织级共享配置为 `RETAIN_BROADER_SCOPE`，禁止在单工作区清理中扩大到全组织删除；对象、向量、缓存、离线、消息、Scheduler 和 Process Memory 为 `EXTERNAL_NOT_EXECUTED`。后三类必须在 impact plan 中可见，不得用“计数为 0”伪装已清理。Connector 旧表的 `workspace_id` 仅在 adapter 边界等价映射为 canonical `project_id`，不改变 scope 值。
3. **dry-run 零写入**：生成计划时只执行带完整 `org_id/project_id` 谓词的计数和 PostgreSQL 目录检查，不调用现有 best-effort 删除接口。执行前重算计划并比对 `planHash`，变更后的旧计划必须失败关闭。
4. **仅允许临时 synthetic 租户执行**：apply 必须同时满足数据库名为 `aos_ti6_*`/`aos_test_*`、组织为 `ti6-synthetic-*`、工作区为 `ti6-synthetic-*`。`dev-org/dev-workspace`、`org-org/org-project`及所有其他 scope 一律拒绝 apply；dry-run 仍可用于只读影响分析。
5. **apply/verify/rollback 不落地删除**：在一个事务中对可清理当前态执行 scoped delete，校验本 scope 计划行数归零且其他 scope 摘要不变，随即回滚到 savepoint，再校验行数与内容摘要完全恢复。同一计划连续执行两次的结果必须一致，作为幂等重试证据。
6. **备份恢复独立验证**：共享库只做 `pg_dump` 备份，恢复到新建临时库后运行 registry/schema 对账和 synthetic drill，不覆盖共享库。临时库可在验证完成后删除，备份文件保留为本波证据。
7. **结论边界**：PostgreSQL synthetic drill GREEN 不等于生产组织删除能力 GREEN。未完成真实后端 PROBED 的非 PostgreSQL 资源、TI-1 E4 历史隔离以及既有未知前缀继续是生产门 blocker，但不阻止“本地微商城 Connector 开发前置”收口。

TI-6-4 验收项：注册表校验 GREEN；dry-run 确定且零写入；保护 scope apply 失败关闭；临时 synthetic 租户两次 apply/verify/rollback 摘要一致；跨租户摘要不变；备份恢复库 schema/registry 无漂移；全过程对共享库业务行零改动。

TI-6-4 已 GREEN：代码 `44887c1` 新增注册表驱动的确定性 impact plan 和只限 `aos_test_*/aos_ti6_* + ti6-synthetic-*` 的 rollback-only 演练器。栖月汇 dry-run 业务行为 0；完整 ACL 备份恢复库 101/101 注册覆盖、schema drift=0，synthetic apply/verify/rollback 两次结果一致且 peer 不变。Tenant Isolation 239 passed / 10 skipped，五分支同线。下一门 TI-6-5 最终对账。

TI-6-5 已完成：Schema/Resource 10 passed，OpenAPI/route 38 passed + 2 subtests，Tenant Isolation 239 passed / 10 skipped，全量 9,235 collected；OpenAPI 2,284 paths、4,047 route rows、4,028 unique operation pairs，产物无漂移。栖月汇复扫为业务 0、active Module 0、目标非表资源 0，`localDevelopmentReady=true`。因 TI-1 E4、未归属/空 scope、未知前缀和外部后端未完成 PROBED，`productionDeploymentReady=false`。本方案的本地接入前置已 GREEN，按约定在真实微商城 Connector/API/凭据编码前暂停。

TI-6-1 已 GREEN：代码 `81db2f0` 将 27 张已完成迁移表的机器登记校准为实库状态，并登记 6 张 ownership ledger/orphan quarantine 维护表；实库 coverage 为 101/101、unregistered/stale/drift 均为 0。预检每表使用 savepoint，单个登记错误不会再把后续 100 张表的只读证据连锁污染。Tenant Isolation 220 passed / 8 skipped，全量 9,213 collected，五分支同线。下一门 TI-6-2 只处理重扫确认的 6 个弱主键、7 个复合 FK 缺口、1 类历史空 scope 及非 PostgreSQL 维护命名识别。

#### TI-6-2 细分实施门（2026-08-05）

1. **TI-6-2A 预检语义校正**：对象存储 `_maintenance/quarantine/unowned/` 是不可变维护隔离区，不算活跃未知前缀；`vector_index:{org}__{project}__{logical}` 按已冻结 collection contract 聚合到对应租户；`twa_workspace/meta_workspace` 是工作区根，不要求自引用 workspace FK。严格生产证明仍把 NOT_CONFIGURED/STATIC_ONLY 单列，不改变其事实状态。
2. **TI-6-2B 可逆结构 Contract**：`theme/widget_catalog` 的测试组织行、空的 `twa_audit/twa_invite/twa_join_request` 按 scoped 复合主键、workspace FK、NOT NULL 和 RLS 收口；先做临时库 downgrade/upgrade 与 count/hash 守恒。`authz_tuple` 8 条空 scope 属 TI-1 E3 历史断点，不在本波静默归属或改主键，继续由逻辑隔离和 E4 BLOCKED 管理。
3. **TI-6-2C 剩余 RLS/关系门**：资产安装/Integration Case、组织目录和 TI 控制面 ledger 等已有强 scoped key 的 tenant-owned 表补齐 runtime transaction 与 ENABLE/FORCE RLS；能由权威 `twa_workspace` 证明的 FK 才 Validate。`meta_membership → meta_workspace` 当前 53 条投影断链必须先修复投影生成机制，禁止为过门删除 membership 或伪造栖月汇数据。
4. **双结论**：本地开发微商城前置门允许“业务数据 0 + 已配置本地后端实测 + 外部生产依赖明确 NOT_CONFIGURED”；生产门仍要求所有后端 PROBED、TI-1 E4 和目录投影断链关闭。两者不得用同一个 GREEN 文案混淆。

TI-6-2A 已 GREEN：代码 `22cf8d7` 将 2 个不可变维护隔离对象从活跃未知前缀中分离，并按冻结的 `{org}__{project}__{logical}` 契约识别 1 个测试组织 vector namespace；向量 unknown 从 1 降为 0，对象 unknown 从 5 收敛为 3（`org-a` 2、`dev-probes` 1，继续阻断且未删除）。Tenant Isolation 221 passed / 8 skipped。下一门 TI-6-2B。

TI-6-2B 已 GREEN：代码 `e1e3d7b` 通过 `228ti6bcontract` 将 `theme/widget_catalog/twa_audit/twa_invite/twa_join_request` 收口为 scoped 复合主键和已验证 workspace FK；Workshop 两表 ENABLE/FORCE RLS，同一逻辑 ID 可在不同组织分别定制。临时库 downgrade/upgrade 与共享库升级前后 3/17/0/0/0 行及 hash 守恒；共享备份位于数据库容器 `/tmp/aos-ti6-2b-before.dump`。实库状态收敛为 80 STRONG、3 WEAK（其中唯一 tenant-owned WEAK 为 E3 历史 `authz_tuple`），结构 blocker 从 6/7 降为 1/5。Tenant Isolation 224 passed / 8 skipped，全量 9,217 collected。下一门 TI-6-2C。
...
评审通过后的 **TI-0**、TI-PRE-1、TI-2～TI-5 与 TI-6-1 已完成。TI-1 E4 仍因 59 条逻辑隔离记录 BLOCKED；按连续执行授权，下一步执行 TI-6-2～TI-6-5，并在本地微商城接入准备 GREEN 后、真实 Connector 编码前暂停。

### 20.1 TI-6-2D 关系真值门（实施冻结）

TI-6-2D 不重新设计租户模型，只修正注册表语义并补齐资产控制聚合根到 canonical 工作区的数据库关系：

1. `meta_workspace`、`twa_workspace` 是工作区根节点，不应再被通用 precheck 要求反向引用自身；注册表新增 `workspaceRoot: true`，loader 只允许它出现在 `TENANT_OWNED + STRONG_PK + CONTROL_PLANE` PostgreSQL 表。
2. `bundle_composition`、`bundle_installation_command`、`integration_case` 是当前预检识别出的三类无直接工作区父关系的资产聚合/命令根；新增已验证 `(org_id, project_id) → meta_workspace(org_id, project_id)` FK，不改变现有主键、业务 payload、状态机或 API。
3. 其他表继续以直接 workspace FK 或 scoped 父子 FK 形成关系闭包；不得用 `workspaceRoot` 掩盖业务表缺失 FK。
4. 迁移只新增/删除三条 FK，不执行历史 UPDATE/DELETE；共享升级前备份，临时库完成 downgrade/upgrade 和行数/hash 守恒。
5. 退出门：`TENANT_COMPOSITE_FOREIGN_KEY_MISSING=0`；`authz_tuple` 空 scope/弱键与 `meta_membership` 53 条 orphan 继续作为真实阻断，不因本波改写。

### 20.2 TI-6-3 空组织与应用实例门（实施冻结）

TI-6-3 按现有 `twa_*` 权威目录、`meta_*` 兼容投影、Module/Overlay 表和卸载 Contract 实施，不引入第二套安装架构：

#### TI-6-3A 目录对齐与机器 readiness

1. 以可逆迁移把 `dev-org` 在 `twa_org/meta_org` 的显示名从“默认组织”改为“测试组织”；只匹配冻结旧值，不覆盖用户已改名称。
2. 将已有 `meta_org/meta_workspace` 的 `org-org/dev-project` 控制面身份补入权威 `twa_org/twa_workspace`；不得复制任何业务表、成员、应用或历史未知数据。
3. 新增只读 readiness 编译器和 CLI，分别输出目录一致性、栖月汇 BUSINESS_DATA、控制面计数、已安装 Module、非 PostgreSQL目标前缀及保留阻断；不得用全局其他租户的未知数据伪称“栖月汇已有数据”。
4. 本地接入门与生产部署门分开：栖月汇业务/应用为 0 且目标 scope 无非表数据，可形成 LOCAL READY；未知历史、外部 NOT_CONFIGURED/STATIC_ONLY、TI-1 E4 继续阻断 PRODUCTION READY。

#### TI-6-3B 四模板与实例定制闭环

1. 四模板标识沿用本文 1.2：`workshop.order-management`、`workshop.risk-inbox`、`workshop.cop-dashboard`、`workshop.buddy-assist`；模板为进程内不可变平台目录，不含租户数据。
2. 新增只读模板列表和安装入口；安装复用 `meta_module + module_instance_overlay + active_overlay_revision/effective_config_hash`，并显式写入 template/version/installation，不另建状态机。
3. Overlay 只允许品牌、页面、字段、指标、流程、智能体、权限和数据绑定等冻结键；服务端计算 base/effective hash，actor 只存 hash。
4. 退出门使用临时 synthetic 租户验证：栖月汇初始安装数 0；同一模板跨组织相同 moduleId 可安装且不同 Overlay；卸载只影响当前 scope；测试组织实例和 `obj_instance` 业务事实 hash 不变。
5. 不在共享栖月汇写入 synthetic Module；真实微商城 Bundle、凭据、账号、订单和 Connector 仍禁止进入。
...
| v1.13 | 2026-08-04 | E3-0R 确认历史记录身份不可恢复；采用历史断点 + 当前执行基线双轨，并记录连续完成非生产 E3 与微商城前置准备的授权边界 |
| v1.14 | 2026-08-04 | E3 非生产工程收口：append-only ledger、59 条 quarantine、恢复库可逆演练和共享非生产零回填完成；历史连续性风险接受，E4 继续 BLOCKED |
| v1.15 | 2026-08-04 | TI-PRE-1 收口共享 PostgreSQL 测试隔离：memory 真隔离、pytest 临时数据库和共享库 hash 守卫 GREEN |
| v1.16 | 2026-08-04 | TI-2 E1 Module/Overlay nullable Expand GREEN：95 张 PostgreSQL 表、102 个资源、644 行守恒和真实迁移可逆 |
| v1.17 | 2026-08-04 | 冻结 TI-2 E2 分片：A=Canvas/Widget/Variable，B=Query/Event/Interface，C=Deployment/Theme/Catalog，D=aggregate/default gate |
| v1.18 | 2026-08-04 | TI-2 E2-A GREEN：Canvas/Widget/Variable 与 Query list 显式 TenantScope，53/77 项累计测试通过 |
| v1.19 | 2026-08-04 | TI-2 E2-B GREEN：Query 写、Event CRUD、Interface 读写显式 TenantScope，47/80 项累计测试通过；下一步 E2-C |
| v1.20 | 2026-08-04 | TI-2 E2-C GREEN：Deployment、Theme、Widget Catalog 与 usage/ref 链显式 TenantScope，46/91 项累计测试通过；下一步 E2-D |
| v1.21 | 2026-08-04 | TI-2 E2-D 与 E2 总收口：Module aggregate TenantScope、10 Store 默认租户静态门，56/87 项累计测试通过；APP-04/05 保持未完成，下一步 E3 |
| v1.22 | 2026-08-04 | TI-2 E3 GREEN：确定性身份回填 622、quarantine 2、恢复库与共享库可逆演练、最终 verify 和新写持续性门完成；下一步 E4 |
| v1.23 | 2026-08-04 | TI-2 E4 GREEN：11 个 Module 身份 FK 零违规 Validate，真实升降级可逆、行数守恒、栖月汇 Module 0、RLS 0；下一步 E5 Read Switch |
| v1.24 | 2026-08-04 | TI-2 E5 GREEN：Module 父子读改删切稳定 module_pk，影子对读失败关闭，130 项累计测试通过；下一步 E6 RLS |
| v1.25 | 2026-08-04 | TI-2 E6 GREEN：真实降权运行角色、11 表 ENABLE/FORCE RLS、无 GUC零可见、WITH CHECK 失败关闭与 134 项累计测试；下一步 E7 Contract |
| v1.26 | 2026-08-04 | TI-2 E7 与 TI-2 总收口 GREEN：8 表复合主键/NOT NULL、2 条 orphan 维护隔离、APP-04/05、软卸载与真实升降级往返完成；下一执行域 TI-3 |
| v1.27 | 2026-08-04 | TI-3 E1 GREEN：5 表 nullable scope、9 个 NOT VALID workspace FK、1,030 行守恒和真实升降级往返完成；下一门 E2 |
| v1.28 | 2026-08-04 | TI-3 E2-A/E2-B GREEN：Object/Graph/Branch mutation 与 Draft/Wiki/Lifecycle/TTL 新写显式 TenantScope，跨 scope 遗留全局键失败关闭；下一门 E2-C |
| v1.29 | 2026-08-04 | TI-3 E2-C 与 E2 总收口 GREEN：Object Runtime 全部目标读链、聚合和静态门显式 TenantScope；下一门 E3 历史归属与隔离 |
| v1.30 | 2026-08-04 | TI-3 E3 GREEN：1,030 行逐记录账本，NO_ACTION 993、QUARANTINE 37、零业务 DML，恢复库与共享最终验证完成；下一门 E4 |
| v1.31 | 2026-08-04 | TI-3 E4 GREEN：9 个 workspace FK 可逆 Validate，1,030 行/37 条 quarantine 守恒，累计回归 GREEN；下一门 E5 Read Switch |
| v1.32 | 2026-08-04 | TI-3 E5 GREEN：Object Runtime 全包读旁路清零，vector/demo scope 收口，TI-4 Connector writer 精确阻断；下一门 E6 RLS |
| v1.33 | 2026-08-04 | TI-3 E6 GREEN：9 表 ENABLE/FORCE RLS、双 GUC policy、无 GUC零可见与可逆演练完成；下一门 E7 Contract |
| v1.34 | 2026-08-04 | TI-3 E7 评审通过并执行：冻结 9 表 scoped 复合主键、37 条未知归属不可变隔离、scoped branch FK 与既有写路径映射；Connector 继续留在 TI-4 |
| v1.35 | 2026-08-04 | TI-3 E7 与 TI-3 总收口 GREEN：9 表复合 Contract、37 条可逆隔离、同 ID 双租户、真实往返、160 项累计门与五分支同步完成；下一执行域 TI-4 |
| v1.36 | 2026-08-04 | TI-4 C1 GREEN：5 张电商真源表新增 workspace alias NOT VALID FK，零业务 DML、真实往返、126 项累计门与五分支同步完成；下一门 C2 |
| v1.37 | 2026-08-04 | TI-4 C2 GREEN：五类通用 Connector 显式 TenantScope、scoped Object upsert、deferred writer 清零，132 项累计门与五分支同步完成；下一门 D1 |
| v1.38 | 2026-08-04 | TI-4 D1 GREEN：Data OS 10 个 nullable scope 列、7 个 NOT VALID workspace FK、297 行守恒、真实往返与 135 项累计门完成；下一门 D2 |
| v1.39 | 2026-08-04 | TI-4 D2 GREEN：六类 persist、Graph 与删除链显式 TenantScope，boot 无 scope 物理删除停止；297 行守恒，下一门 D3 历史归属逻辑隔离 |
| v1.40 | 2026-08-04 | TI-4 D3 GREEN：297 条逐记录归属账本，4 NO_ACTION、293 QUARANTINE、零业务回填；下一门 D4 workspace FK Validate |
| v1.41 | 2026-08-04 | TI-4 D4 GREEN：7/7 Data OS workspace FK validated，真实往返与 297/293 守恒；下一门 D5 scoped read/boot |
| v1.42 | 2026-08-04 | TI-4 D5 GREEN：Data OS scoped read、lazy boot、严格工作区 API，293 NULL 默认不可见；下一门 D6 RLS |
| v1.43 | 2026-08-04 | TI-4 D6 GREEN：7 表 FORCE RLS、双 GUC policy、无 GUC/跨 scope 失败关闭和真实可逆演练完成；下一门 D7 Contract |
| v1.44 | 2026-08-04 | TI-4 D7 与 Data OS 总收口 GREEN：293 条不可变隔离、7 表 scoped Contract、6 条父子 FK、迁移真源与真实可逆演练完成；下一门 C3 Connector Contract |
| v1.45 | 2026-08-04 | TI-4 C3 与 TI-4 总收口 GREEN：5 表 FK Validate/FORCE RLS、SQLAlchemy/OAuth scoped transaction、209 项累计门和五分支同步完成；下一执行域 TI-5 |
| v1.46 | 2026-08-04 | 对账纠正：C3 与 Data OS/Connector GREEN 不等于 TI-4 全域完成；冻结组 ti4-async-runtime 尚待 A1，先补 Apollo Spoke 与 Scheduler scope |
| v1.47 | 2026-08-04 | TI-4 A1 与 TI-4 全域收口 GREEN：Scheduler 内存状态和 Apollo Spoke 均完成完整 TenantScope，`228ti4a1apollo` 可逆往返、176 项租户回归与五分支同步完成；下一执行域 TI-5 |
| v1.48 | 2026-08-04 | TI-5 A1 GREEN：7 张 AIP 运行表 workspace FK/FORCE RLS、四类 Store scoped transaction、27 行可逆守恒、83 项 AIP 与 181 项租户回归完成；下一门 A2 |
| v1.49 | 2026-08-04 | TI-5 A2 GREEN：23 条 AIP KV 显式清单归测试组织，账本、workspace FK、复合主键、FORCE RLS、scoped runtime 与可逆往返完成；下一门 A3 |
| v1.50 | 2026-08-04 | TI-5 A3 GREEN：631 条 lineage 逐条以唯一 Draft 父记录归属，0 quarantine、0 orphan，复合 FK/PK、FORCE RLS、scoped runtime 与真实可逆往返完成；下一门 B1/B2 |
| v1.51 | 2026-08-04 | TI-5 B1 GREEN：七张模型管理表 65 行完成复合 PK/workspace/父子 FK/FORCE RLS、显式 scoped runtime 与真实可逆往返；下一门 B2 |
| v1.52 | 2026-08-04 | TI-5 B2 GREEN：磁盘 Provider manifest 平台模板与 AIP KV 组织安装/配置实例分离通过跨 scope 负向验证；下一门 C |
| v1.53 | 2026-08-04 | TI-5 C1 GREEN：对象探针仅列认证 scope 前缀，2 个无归属 probe 可逆维护隔离，local vector namespace 通过契约验证；下一门 C2 |
| v1.54 | 2026-08-04 | TI-5 C2-A GREEN：AIP Catalog 与 Phase5 Dataset/Build/Health/SyncConfig 完整 scoped key、Principal 与负向门；下一门 C2-B |
| v1.55 | 2026-08-05 | TI-5 C2-B 与 C2 总门 GREEN：wave_ext Dataset/Media scoped key、GET `/v1/datasets` 真源、211 项租户回归与五分支同步；下一门 C3 |
| v1.56 | 2026-08-05 | TI-5 C3-A GREEN：Job/DLQ、DocIntel 失败路径与 demo 状态使用完整 scoped key，相关回归和五分支同步完成 |
| v1.57 | 2026-08-05 | TI-5 C3-B/C GREEN：Phase5 进程态、Schedule/Run 与非表资源机器分类收口；未配置后端保持显式 NOT_CONFIGURED |
| v1.58 | 2026-08-05 | TI-5 D 最终 GREEN并冻结 TI-6-0R～6-5：迁移可逆、共享库 fingerprint、栖月汇 TI-5 零数据及本地/生产双门 |
| v1.59 | 2026-08-05 | TI-6-1 GREEN：101/101 实库登记对账、27 张状态校准、6 张维护表补登记、precheck savepoint 隔离与零 drift 门完成；下一门 TI-6-2 |
| v1.60 | 2026-08-05 | TI-6-2A GREEN：维护对象隔离计数、vector scoped namespace 聚合与非表未知项精确收敛；3 个真实未知对象继续阻断 |
| v1.61 | 2026-08-05 | TI-6-2B GREEN：5 张剩余基础表 scoped PK/FK、Workshop RLS、跨组织同逻辑 ID 与真实可逆守恒完成；下一门 TI-6-2C |
| v1.62 | 2026-08-05 | TI-6-2C GREEN，并冻结 TI-6-2D：工作区根节点显式语义、三类资产聚合/命令根直连 canonical workspace FK，不改变既有资产架构 |
| v1.63 | 2026-08-05 | TI-6-2D GREEN：工作区根节点语义生效、三条资产根 workspace FK 已验证，预检复合 FK 缺口归零；下一门 TI-6-3 实例 readiness |
| v1.64 | 2026-08-05 | 冻结 TI-6-3A/3B：对齐测试组织与栖月汇权威目录，生成本地/生产双 readiness，并复用 Module/Overlay 完成四模板安装定制闭环 |
| v1.65 | 2026-08-05 | TI-6-3A GREEN：双目录对齐，栖月汇业务/已安装应用/目标非表资源均为 0，LOCAL READY；生产阻断如实保留，下一门 3B |
| v1.66 | 2026-08-05 | TI-6-3B GREEN：四个不可变平台模板、scoped Module/Overlay 安装、effective config 回读、跨组织定制与卸载保留业务事实完成；下一门 TI-6-4 |
| v1.67 | 2026-08-05 | TI-6-4 GREEN：注册表 impact plan、synthetic 双次 rollback-only drill、完整 ACL 备份恢复和共享库零写入完成；下一门 TI-6-5 |
| v1.68 | 2026-08-05 | TI-6-5 最终对账完成：本地微商城 Connector 开发前置 GREEN，生产门 YELLOW；在真实平台接入前暂停 |
