...
2. **三类处置语义不混合**：普通当前态为 `DELETE_SCOPED_ROWS`；存在 `DELETE/TRUNCATE` 不可变防护的审计、修订和证据表为 `RETAIN_IMMUTABLE_HISTORY`；对象、向量、缓存、离线、消息、Scheduler 和 Process Memory 为 `EXTERNAL_NOT_EXECUTED`。后两类必须在 impact plan 中可见，不得用“计数为 0”伪装已清理。
