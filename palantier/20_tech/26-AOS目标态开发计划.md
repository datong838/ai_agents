...
> **版本**：v1.98 · 2026-07-19  
> **状态**：本地主链 ✅ · Apollo 运维深水 ✅ · **Ferry 现场加严 MVP ✅** [162](162-Ferry现场加严MVP方案.md)（≠ 客户气隙签收 / Full Channel）· 更远停车场须点名
> **对齐**：[20](20-AOS整体技术方案.md) · [20a](20a-多用户与工作区整站隔离方案.md) · [20b](20b-AOS端云分离与交付形态方案.md) · [20c](20c-AOS桌面端详细技术方案.md) · [T-EVO](T-EVO-v0.1到目标态替换阶梯.md) · [00 索引](00-技术方案索引.md) · [23](23-AOS开源引用与交付军规.md) · [24](24-AOS客户侧前置组件安装SOP.md) · [07b](../07b-Capability-Adapter重能力接入.md) · [T-UI](T-UI-前端工程与foundry-html落地规范.md) · **[27](27-本机开发基础设施与工程门禁记录.md)** · **[70](70-业务平台可演示优先计划.md)** · **[72](72-系统启停与健康检查手册.md)** · **[73 产品1.3](73-产品1.3分析建模下一阶段方案.md)**  
> **不替代**：各 T0x / 07b / 20a / 20b / 20c 技术详稿（本文只定任务切分与先后）
...
| ☐   | [20a](20a-多用户与工作区整站隔离方案.md) · [20b](20b-AOS端云分离与交付形态方案.md) · [20c](20c-AOS桌面端详细技术方案.md) | 工作区/端云/桌面目标态；**实现见 §14 TWA/TWB/TWC**（方案已定 · 编码另开） |
| ☐   | 本文 **26**（含 §14）                                                           | §4 推荐总序；§6「现在不要干」；§11.2 显式延期；§14 TWA/TWB/**TWC** 进度看板 |
...
| **TA.*** | 产品 1.3       | 分析建模（§13） |
| **TWA.*** | Wave-W / 20a | **工作区整站隔离**（§14） |
| **TWB.*** | Wave-W / 20b | **端云分离与运维面**（§14） |
| **TWC.*** | Wave-W / 20c | **桌面端**（§14.6；UI-01～16） |
...
| **未点名就开 §14 编码** | 方案已定 · 见 **§14.9**；须点名具体 TWA/TWB/TWC ID |
| 端业务壳以 Apollo 为一级主导航 | 20b §5：Apollo → **运维面** |
...
| 下一刀建议？ | **方案已齐 · 须点名任务 ID**（§14.9）；荐 **TWA.1**，可并行 **TWC.1**；见 §14.7 |
...
**2026-07-19 增补：** 工作区整站隔离 + 端云 + **桌面端**属 **目标态下一主节点**，任务与进度见 **§14**（TWA/TWB/**TWC**；方案 20a/20b/20c 已定稿；编码默认 **待编码**，须人点名开刀）。
...
| **工作区隔离（20a）** | 多用户 · 切换 · 整站双字段 | 字段/OpenFGA 有底；**产品化未编码**（§14 TWA） | **低（产品）** |
| **端云/桌面（20b）** | 三层 UI · 桌面产品位 · SKU | 方案已决；desktop 未收口（§14 TWB） | **低（产品）** |
| 产品 1.3 | Jupyter / R / SQL | MVP 已退出 [118]；真 Notebook/BI 全集后置 | **中（MVP）** |
...
| 档 | 项 |
| --- | --- |
| **近** | **§14 TWA P0 / TWB E1 / TWC D1～D2**（工作区 · Web 端云 · 桌面收口）；生产 IdP 真 token 联调 |
| **中** | 20a P1～P2；TWB E3 离线/更新；Full Spoke 运行时；正式安装包；`ontology-sdk` |
| **远** | 20a P3 多 Org；TWB E4 SaaS 开通台；200+ Connector；真 PaddleOCR GPU；Iceberg；Scenario |
...
> **状态**：✅ **阶段退出**（2026-07-18）· TA.0～8 ✅ · 收口 [118](118-产品1.3分析建模阶段退出收口.md) · **下一刀须人点名**（真 Jupyter / BI / Apollo 加深）
...
## 14. 下一阶段：工作区隔离 + 端云交付 + 桌面端（目标态 · 20a/20b/20c）

> **决策日**：2026-07-19 · **详稿**：[20a](20a-多用户与工作区整站隔离方案.md) **v1.0.4** · [20b](20b-AOS端云分离与交付形态方案.md) **v1.1.2** · [20c](20c-AOS桌面端详细技术方案.md) **v1.0.2**  
> **定位**：**目标态**产品化主节点（Wave-W）；挂在 Wave-0～5 / §12 / 1.3 MVP **之后**  
> **状态**：方案 ✅ 已定稿 · **编码门禁见 §14.9**（须点名具体任务 ID）· 进度见下表  
> **地基可复用（不算产品化完成）**：Principal 已有 `org_id`/`project_id`；TX.3/TX.4 · OpenFGA；Web AppShell/`nav.ts`；`mybuddy/desktop` Tauri 壳可迁 — **缺**工作区产品化、端导航收口、`apps/desktop` 同构座舱

### 14.1 目标态一句话

| 轴 | 目标态 |
| --- | --- |
| **20a** | 标准企业 = **单 Org · 多工作区**；用户可切换；列表/读写/对象存储前缀 **整站隔离**；种子进 **测试工作区**；无演示壳 |
| **20b** | **端** = 业务座舱 + 工作区管理（Web/桌面同构）；**运维面** = Apollo/集群/开通；桌面为两 SKU **标准项**；Local-First 产品名 = **本机平台** |
| **20c** | **AOS 桌面**：Tauri 壳 + 与 Web **同构且功能 ≥ Web**（`nav.ts` 全量）；托盘/深链/离线/更新；默认首页=概览 |

### 14.2 硬规则

| 规则 | 说明 |
| --- | --- |
| 先方案后刀 | 20a/20b/20c 已定；**大刀**（跨多服务/新包）另开子方案号；**小刀**可直接按本表 DoD 开写 |
| 契约不双轨 | 工作区 = `project_id`；禁止另造 `workspace_id` |
| R-PROD | 无 demo 临时页 / `if demo` 主路径；禁止「演示桌面」精简壳 |
| R-ARCH | Web/Desktop **只**调 `aos-api` |
| UI 三层 | 业务①② 在端；Apollo **可收不少页**（20b + 20c §5.4.3） |
| 桌面同构 | 共享 `nav.ts`/路由；**禁止** desktop 业务页双仓 |
| **≥ Web** | R-PARITY-01（20c §5.4 · D-A11） |
| 编码门禁 | 见 **§14.9**；按 **§14.7** 序；禁止跳过硬依赖 |
...
| --- | --- | --- |
| **方案** | 已定稿 / 待补刀 | 详稿或实现子方案 |
| **编码** | **待编码** · **已编码** | 产品化代码是否合入主仓 |
| **测试** | **待测试** · **已测试** | 对应该任务 DoD 的自测/pytest/手工验收 |

> §10 = 历史 Wave；**§14 = 目标态新节点真源**。

### 14.4 TWA.* · 工作区整站隔离（对齐 20a P0～P3）

| ID | 任务 | 目标态 DoD（摘要） | 依赖（硬） | 方案 | 编码 | 测试 |
| --- | --- | --- | --- | --- | --- | --- |
| **TWA.0** | 方案/索引挂接 · 口径冻结 | 20a 已挂 00/26；工作区=Project | — | **已定稿** | — | — |
| **TWA.1** | 安全收紧 · 禁生产 Dev 越权头 | Header≠claim 拒；伪造 `X-Project-Id` 不能进无成员区（A3） | TWA.0, TX.3 ✅ | **已定稿** | **已编码** | **已测试** |
| **TWA.2** | 会话上下文门面 | 去掉写死 `dev-org`/`dev-project`；`/v1/me` 注入；种子→**测试工作区**（A8） | TWA.1 | **已定稿** | **已编码** | **已测试** |
| **TWA.3** | 壳工作区切换器 | 「工作区」下拉；切区后主列表换区（A2） | TWA.2 | **已定稿** [123](123-TWA3-壳工作区切换器方案.md) | **已编码** | **已测试** |
| **TWA.4** | 隔离门禁测试夹具 | OrgA/Prj1 vs Prj2 越权 pytest（A1） | TWA.2 | **已定稿** [124](124-TWA4-隔离门禁测试夹具方案.md) | **已编码** | **已测试** |
| **TWA.5** | 漏网表/读路径盘点销项 | 双字段谓词台账清零（A4） | TWA.4 | **已定稿** [125](125-TWA5-漏网表读路径盘点销项方案.md) | **已编码** | **已测试** |
| **TWA.6** | 插件：目录共用 · 配置按区 | 已配置实例绑 `project_id`（A9） | **TWA.3, TWA.5** | **已定稿** [126](126-TWA6-插件目录共用配置按区方案.md) | **已编码** | **已测试** |
| **TWA.7** | P1 成员与管理面 | Members API/页；Audit（A5） | TWA.3, TWA.4 | **已定稿** [127](127-TWA7-工作区成员与审计方案.md) | **已编码** | **已测试** |
| **TWA.8** | P2 存储/检索前缀门禁 | MinIO/向量/Wiki 前缀 | TWA.5 | **已定稿** [142](142-TWA8-存储检索前缀门禁方案.md) | **已编码** | **已测试** |
| **TWA.9** | P3 多 Org / Hub+Spoke | Org 切换；Spoke 绑 Org（**后置**） | TWA.7, T5.* | **已定稿** [143](143-TWA9-多Org与Spoke绑定方案.md) | **已编码** | **已测试** |
| **TWA.10** | 建组织/工作区 · 邀请/申请 | 顶栏新建；邀请链接；非成员申请审批 | TWA.7, TWA.9 | **已定稿** [164](164-TWA10-组织工作区创建与邀请审批方案.md) | **已编码** | **已测试** |
| **TWA.11** | 删组织/工作区 · 清数据 | 非空 409；管理员可 clear 后再删 | TWA.10 | **已定稿** [166](166-TWA11-组织工作区删除与清数据方案.md) | **已编码** | **已测试** |
| **TWA.12** | 成员邮箱/手机识别 | `email:`/`phone:` subject；验证码后置 | TWA.7 | **已定稿** [168](168-成员自然人识别-邮箱手机号方案.md) | **已编码** | **已测试** |

**P0 退出** = TWA.1～TWA.6 **已编码 + 已测试**。

### 14.5 TWB.* · 端云分离与运维面（对齐 20b E0～E5）

> 桌面见 **§14.6**。本节 = Web/产品位收口。

| ID | 任务 | 目标态 DoD（摘要） | 依赖（硬） | 方案 | 编码 | 测试 |
| --- | --- | --- | --- | --- | --- | --- |
| **TWB.0** | 端云口径定稿 | E0 已决 | — | **已定稿** | — | — |
| **TWB.1** | Web 可配 API Base | 多环境 Base；无引擎直连（C3/C4） | **TWA.3**, TWB.0 | **已定稿** [128](128-TWB1-Web可配API-Base方案.md) | **已编码** | **已测试** |
| **TWB.2** | 端导航：Apollo 默认可收 | ①② 在壳；环境只读；配模型不碰 Vault（C8/C9）；**路由不少** | TWB.0 | **已定稿** [129](129-TWB2-Apollo导航默认可收方案.md) | **已编码** | **已测试** |
| **TWB.3** | Web 文案「本机平台」 | 不叫 Apollo（C1/C10） | TWB.0 | **已定稿** [130](130-TWB3-本机平台文案方案.md) | **已编码** | **已测试** |
| **TWB.6** | E4 SaaS 开通台 | 多 Org 开通·配额（**后置**） | TWA.9, TWC.2 | **已定稿** [144](144-TWB6-SaaS开通台方案.md) | **已编码** | **已测试** |
| **TWB.7** | E5 Hub/Spoke·气隙端 | Ferry↔端版本矩阵（**后置**） | T5.*, TWC.2 | **已定稿** [145](145-TWB7-Ferry端版本矩阵方案.md) | **已编码** | **已测试** |

**说明：** TWB.3 与 TWC.10 **文案对齐、无硬依赖**（谁先合入以 PR 互检即可）。  
**E1 退出** = TWB.1～TWB.3 **已编码 + 已测试**（建议同时 TWA P0 已退出）。

### 14.6 TWC.* · 桌面端（对齐 20c D0～D4）

> 详稿：[20c](20c-AOS桌面端详细技术方案.md) v1.0.2；验收含 **D-A11（≥ Web）**。

| ID | 阶段 | 任务 | 目标态 DoD（摘要） | 主要 UI | 依赖（硬） | 方案 | 编码 | 测试 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **TWC.0** | D0 | 详稿挂接 | 20c/索引/§5.4 冻结 | — | TWB.0 | **已定稿** | — | — |
| **TWC.1** | D1 | 工程迁入 `apps/desktop` | 可 `tauri build`；产品名 AOS 桌面 | — | TWC.0 | **已定稿** | **已编码** | **已测试** |
| **TWC.2** | D1 | 同构主壳 · **≥ Web** | 共享 `nav.ts`；差集空；只调 aos-api | UI-01～04·06·07·16 | **TWC.1, TWA.2** | **已定稿** [131](131-TWC2-桌面同构主壳方案.md) | **已编码** | **已测试** |
| **TWC.3** | D1 | 登录与安全存储 | OIDC/回调；Refresh→钥匙串 | UI-03 | TWC.2, TX.3 ✅ | **已定稿** [132](132-TWC3-登录与安全存储方案.md) | **已编码** | **已测试** |
| **TWC.4** | D2 | 托盘·关于·平台徽章 | D-A4；连通灯 | UI-07·08 | TWC.2 | **已定稿** [133](133-TWC4-托盘关于平台徽章方案.md) | **已编码** | **已测试** |
| **TWC.5** | D2 | 深链 `aos://` | 白名单；未登录先登录 | UI-15 | TWC.3 | **已定稿** [134](134-TWC5-深链aos方案.md) | **已编码** | **已测试** |
| **TWC.6** | D2 | 桌面工作区切换 | 同 TWA.3；切区清缓存 | UI-05·12 | TWC.2, **TWA.3** | **已定稿** [135](135-TWC6-桌面工作区切区清缓存方案.md) | **已编码** | **已测试** |
| **TWC.7** | D2 | Apollo 可收不少页 | admin 展开同全七页 | UI-04·14 | TWC.2, **TWB.2** | **已定稿** [136](136-TWC7-桌面Apollo可收不少页方案.md) | **已编码** | **已测试** |
| **TWC.8** | D3 | 离线只读+待同步 | T08 §4.5；禁离线直写 | UI-09 | TWC.2, **TWC.6** | **已定稿** [137](137-TWC8-离线只读待同步方案.md) | **已编码** | **已测试** |
| **TWC.9** | D3 | 签名更新 | 验签失败拒装 | UI-10 | TWC.4 | **已定稿** [138](138-TWC9-签名更新方案.md) | **已编码** | **已测试** |
| **TWC.10** | D4 | 本机平台面板 | 文案禁 Apollo；链 72 | UI-11 | TWC.2, TWB.3 | **已定稿** [139](139-TWC10-本机平台面板方案.md) | **已编码** | **已测试** |
| **TWC.11** | D4 | Buddy 三栏并存 | 非默认首页 | UI-13 | TWC.2 | **已定稿** [140](140-TWC11-Buddy经典三栏并存方案.md) | **已编码** | **已测试** |
| **TWC.12** | D4 | 渠道包 | 预置 Base | UI-01/02 | TWC.2 | **已定稿** [141](141-TWC12-渠道包方案.md) | **已编码** | **已测试** |

| 退出点 | 条件 |
| --- | --- |
| **D1** | TWC.1～3 已编码+已测试（含 D-A11） |
| **D2** | TWC.4～7 已编码+已测试（建议 TWA P0 已退出） |
| **D3** | TWC.8～9 已编码+已测试 |
| **D4** | TWC.10～12 已编码+已测试 |
...
flowchart TB
  ...
  subgraph waveB [Web 端云 · 可早并行]
    TWB2[TWB.2 导航可收]
    TWB3[TWB.3 本机平台文案]
    TWB1[TWB.1 Web API Base]
    TWB0 --> TWB2
    TWB0 --> TWB3
    TWA3 --> TWB1
  end
...
#### 14.7.2 推荐开刀序（单人 · 含并行窗）

```
【已完成】TWA.0 / TWB.0 / TWC.0

【第 1 刀 · 可并行】
  ★ TWA.1（安全收紧）          ← 推荐首刀
  ∥ TWC.1（桌面工程迁入，仅壳）  ← 不挡业务，可并行
  ∥ TWB.2 / TWB.3（Web 导航文案，仅需文档）

【第 2 刀】
  TWA.2 →（TWA.3 ∥ TWA.4）
       → TWA.5 → TWA.6          【工作区 P0 退出】

【第 3 刀 · Web E1】
  TWB.1（依赖 TWA.3）

【第 4 刀 · 桌面 D1】
  TWC.2（依赖 TWC.1 + TWA.2）→（TWC.3 ∥ TWC.4）  【D1 退出还须 TWC.3】

【第 5 刀 · 桌面 D2】
  TWC.5（需 TWC.3）
  TWC.6（需 TWA.3）∥ TWC.7（需 TWB.2）          【D2 退出】

【第 6 刀 · 桌面 D3】
  TWC.8（需 TWC.6）→ TWC.9（需 TWC.4）

【按需 / 后置】
  TWC.10～12 · TWA.7～8 · TWA.9 · TWB.6～7
```

#### 14.7.3 关键路径（挡对外口径的最短链）

| 对外口径 | 最短任务链 |
| --- | --- |
| 「标准企业工作区可用」 | TWA.1→2→(3∥4)→5→6 |
| 「Web 端云产品位」 | 上链 + TWB.1～3 |
| 「桌面主座舱可用（≥Web）」 | TWC.1→2→3（+ TWA.2） |
| 「桌面 OS 获得感齐」 | 上 + TWC.4～7（+ TWA.3, TWB.2） |

#### 14.7.4 禁止的跳刀

| 禁止 | 原因 |
| --- | --- |
| 未做 TWA.1/2 就宣称多工作区隔离 | 仍可能 Dev 越权头 / 写死 org |
| 未做 TWA.2 就合 TWC.2 当完成品 | 桌面会话上下文未接 |
| 未做 TWA.3 就合 TWC.6 / 宣称切区 | 无切换器语义 |
| 未做 TWC.6 就合 TWC.8 | 离线缓存易串工作区 |
| 砍 Apollo 路由充「对齐 20b」 | 违反 ≥ Web / D-A11 |
| 并行开 TWA.9 / TWB.6 抢主路径 | 后置项 |

### 14.8 与既有 Wave 关系

| 既有 | 关系 |
| --- | --- |
| Wave-0～5 / §10 | **不回退**；§14 在其上加深 |
| §12 演示优先 | 正式工作区承载；禁新 demo 壳 |
| §13 / 1.3 | MVP 已退出；读路径随 TWA.2/3 · TWC.6 吃工作区 |
| Apollo Full 运维 | 仍后置；不等于删端上 Apollo 页 |
| T-UI S3 | **TWC.2** 落地 |
...
| 20a / 20b / 20c 方案定稿 | ✅ |
| 挂入 00 / 26 §14 / 依赖图自洽 | ✅ |
| Wave-0～5 / TX.3 地基 | ✅ 可复用 |
| **人点名 §14 开干（或点名首刀 ID）** | ✅ **§14 TWA/TWB/TWC 已收口**（含 TWB.7）· 下一主链 §12/70 或产品 1.3 TA.* |

**执行约定（2026-07-19 已决 · 主驾驶）：**

| 约定 | 说明 |
| --- | --- |
| **主驾驶** | 按 §14.7 序推进；**不逐步请示**微观技术选型（仍遵守 20a/20b/20c 硬规则） |
| **依赖** | 缺包/缺工具 **自行安装**（本机/Dev），不拦着等人 |
| **单测** | 每刀须有对应 pytest/vitest（或等价）；不过不宣称完成 |
| **运行日志** | 关键路径打结构化日志（含 `trace_id`/`org_id`/`project_id` 等）；方便排障；禁打密钥明文 |
| **进度回写** | 刀完更新本表「编码/测试」列；阻塞写入可续干的并行项 |
| **首刀** | **TWA.1**；可并行 **TWC.1**（仅迁壳） |

**结论：** 方案与顺序已齐；**开干后 Agent 主驾驶执行**，无需每步决策。
...
| v1.53 | 2026-07-19 | **§14 目标态节点** · 20a/20b · **TWA.*/TWB.*** · 进度列 待编码/已编码/已测试 · 变更记录改 §15 |

---

| v1.54 | 2026-07-19 | 挂 **[20c](20c-AOS桌面端详细技术方案.md)** · TWB.0c/TWB.4/5 对齐桌面 UI 全表 · **暂不编码** |

---

| v1.55 | 2026-07-19 | **§14 合并 20c 开发计划** · 新增 **TWC.0～12**（D0～D4）· 原 TWB.4/5 并入 TWC · 开刀序 §14.7 |

---

| v1.56 | 2026-07-19 | **桌面 ≥ Web**：20c §5.4 全量对照 · TWC.2/7/D-A11 · Apollo 可收不少页 |

---

| v1.57 | 2026-07-19 | **§14 依赖/开刀序维护**：修 TWC.2←TWA.2、TWA.6/TWC.8 硬依赖 · mermaid · **§14.9 编码门禁** · 首刀荐 TWA.1∥TWC.1 |

---

| v1.58 | 2026-07-19 | **主驾驶开干** · **TWA.1** R-ISO-01 Header/claim [121] · **TWC.1** `apps/desktop` 迁入（vitest+vite build+cargo check） |

---

| v1.59 | 2026-07-19 | **TWA.2** 租户上下文 `/v1/me`·workspaceName=测试工作区 · 去散落 Header · vitest+me 日志 |

---

| v1.60 | 2026-07-19 | **TWA.3** 顶栏工作区切换器 [123] · **TWA.4** 内存隔离夹具 [124] · pytest+vitest |

---

| v1.61 | 2026-07-19 | **TWA.5** 租户台账 P0 清零 [125] · `meta_module` 双字段过滤 |

---

| v1.62 | 2026-07-19 | **TWA.6** 插件目录共用/配置按区 [126] · **TWA P0 退出** |

---

| v1.63 | 2026-07-19 | **TWA.7** 成员/Audit [127] · **TWB.1** Web API Base [128] |

---

| v1.64 | 2026-07-19 | **TWB.2** Apollo 可收 [129] · **TWB.3** 本机平台 [130] · **TWB E1 退出** |

---

| v1.65 | 2026-07-19 | **TWC.2** 桌面同构 ≥ Web [131] · `@aos-web` 嵌入 · parity P1 |

---

| v1.66 | 2026-07-19 | **TWC.3** 登录/钥匙串 [132] · **TWC.4** 托盘/关于 [133] · **D1 退出** |

---

| v1.67 | 2026-07-19 | **TWC.5** 深链 aos:// [134] · **TWC.6** 切区清缓存 [135] |

---

| v1.68 | 2026-07-19 | **TWC.7** Apollo 可收不少页 [136] · **D2 退出** |

---

| v1.69 | 2026-07-19 | **TWC.8** 离线只读+待同步 [137] · UI-09 |

---

| v1.70 | 2026-07-19 | **TWC.9** 签名更新 [138] · **D3 退出** |

---

| v1.71 | 2026-07-19 | **TWC.10** 本机平台面板 UI-11 [139] |

---

| v1.72 | 2026-07-19 | **TWC.11** Buddy 三栏 [140] · **TWC.12** 渠道包 [141] · **D4 退出 · TWC 线收口** |

---

| v1.73 | 2026-07-19 | **TWA.8** 存储/检索前缀门禁 [142] · R-ISO-04 |

---

| v1.74 | 2026-07-19 | **TWA.9** 多 Org / Spoke 绑 Org [143] |

---

| v1.75 | 2026-07-19 | **TWB.6** SaaS 开通台 [144] |

---

| v1.76 | 2026-07-19 | **TWB.7** Ferry↔端版本矩阵 [145] · §14 TWA/TWB/TWC 收口 |

---

| v1.77 | 2026-07-19 | **T-SDK.1** `@aos/ontology-sdk` MVP [146] · 主链后非停车场下一刀 |

---

| v1.78 | 2026-07-19 | **Web 迁用 ontology-sdk 首刀** [147] · DraftInbox/Ontology/Graph |

---

| v1.79 | 2026-07-19 | **SDK approve/reject + 余页迁用** [148] · putObject · OT 深页 |

---

| v1.80 | 2026-07-19 | **useJsonGet 钩子迁用** [149] · objects/drafts 字面路径清零 |

---

| v1.81 | 2026-07-19 | **桌面迁用 ontology-sdk** [150] · alias ≥ Web |

---

| v1.82 | 2026-07-19 | **macOS 打包清单** [151] · pack-desktop-mac.sh · probe-prod-idp.sh |

---

| v1.83 | 2026-07-19 | **Linux 打包清单** [152] · pack-desktop-linux.sh · 24 三端打包分轨齐 |

---

| v1.84 | 2026-07-19 | **Ferry 大镜像打包分轨** [153] · pack/probe `.sh` · 对齐 62 |

---

| v1.85 | 2026-07-19 | **生产 IdP Dev 演练** [154] · probe-keycloak-oidc.sh · drill-prod-idp-via-dev-kc.sh |

---

| v1.86 | 2026-07-19 | **Web tsc 门禁清零** [155] · pack 恢复完整 `npm run build` |

---

| v1.87 | 2026-07-19 | **本地伪生产 IdP** [156] · ALLOW_DEV=0 矩阵 · drill :18080 · 客户现场后序微商城 |

---

| v1.88 | 2026-07-19 | **真 Jupyter MVP** [157] · Notebook 7 边车 · ticket→302 · shaped 回退 |

---

| v1.89 | 2026-07-19 | **Full Spoke MVP** [158] · helm-mock · chart stub · 真 K8s 仍延期 |

---

| v1.90 | 2026-07-19 | **BI 子集加深** [159] · Contour/Quiver/Vertex 交互加厚 · 非整 BI 进包 |

---

| v1.91 | 2026-07-19 | **Apollo 运维深水 MVP** [160] · Change/健康晋升/hotfix · ≠ 真舰队 |

---

| v1.92 | 2026-07-19 | **微商城 IdP 验收规程** [161] · accept-customer-idp.sh · 60 §6.3 |

---

| v1.93 | 2026-07-19 | **Ferry 现场加严 MVP** [162] · Unix cosign/skopeo probe · onsite `--sign-mode` · 单测 |

| v1.94 | 2026-07-19 | **TWA.10** 组织/工作区创建与邀请审批 [164] · 顶栏新建 · 申请批准 |

| v1.95 | 2026-07-19 | **本机平台依赖自动拉起** [165] · PG/MinIO ensure · 去演示文案 |

| v1.96 | 2026-07-19 | **TWA.11** 组织/工作区删除与清数据 [166] · 非空禁删 |

| v1.97 | 2026-07-19 | **启停说明端面** [167] · **成员邮箱/手机** [168] · TWA.12 |

| v1.98 | 2026-07-19 | **按钮蓝图对齐扫尾** [169] · 租户面 btn* · 裸钮兜底 |

---

*v1.98 · docs/palantier/20_tech/26 · 按钮蓝图扫尾 [169] · 更远停车场须点名 · 启停见 [72](72-系统启停与健康检查手册.md) **v1.6**（系统端面 [167](167-启停说明系统端面方案.md)）*