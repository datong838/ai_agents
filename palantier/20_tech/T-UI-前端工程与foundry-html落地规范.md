...
| 项 | 策略 |
| --- | --- |
| 详稿 | **[20c · AOS 桌面端详细技术方案](20c-AOS桌面端详细技术方案.md)**（UI-01～16 · 壳 · 迁移 · 验收） |
| Tauri 壳 | 保留；窗口 / 托盘 / 本地服务发现 / 深链 / 更新（见 20c） |
| 内容区 | 与 `apps/web` **同构路由**（共享 app-routes）；默认首页 = 业务概览 |
| Buddy | v0.1 三栏可并存为模式，非默认首页（20c UI-13） |
| `/v1/buddy/ask` | 经 `contracts-client` 调用；上游 Dify 可换（见 T-EVO） |
| 去品牌 | 延续 [10g](../10_v01/10g-交付面去Dify品牌说明.md) |
| 排期 | [26 §14.6 TWC.*](26-AOS目标态开发计划.md) |
