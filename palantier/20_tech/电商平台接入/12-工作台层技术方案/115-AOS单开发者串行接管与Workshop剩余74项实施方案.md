...
| S8 | 0 | 八大菜单前后端视觉一致性封板（治理加固波，不新增 96 项主 Task） | 八页逐组件视觉差异表、后端合同/投影对账、修正、三视口浏览器与累计回归 |
| **合计** | **74** |  |  |

S8 是 W8 发布决定前的新增硬门，不改变原 96 项编号和既有能力范围。前面波次的浏览器证据若只覆盖功能、权限、空态、溢出或交互，而没有逐项对照正式视觉稿，则不得继承为视觉完成。S8 对八大菜单逐页执行：正式视觉稿与产品方案复习、前端布局/组件/状态/响应式差异表、后端合同/权威投影/空态差异表、最小修正、专项测试、全量累计回归、1280/1440/1920 浏览器前后证据、方案一致性复审、Receipt/authority/Prime 回读。前端视觉和后端真实投影任一未闭合，页面不得计为完成。
