...
> 审计日期：2026-08-30
> 状态：`PRODUCT_ACCEPTANCE_REOPENED / R0_4_OF_4 / TOTAL_4_OF_81 / NO_RELEASE`
...
- [ ] `R1-01` 删除八页单独硬编码的缩减菜单源；八页、Buddy 与其他 AOS 页面统一消费同一 canonical `navNodes`/权限投影。
- [ ] `R1-02` 恢复完整菜单树：工作台八菜单、Buddy、应用程序构建工具全部项、AIP 决策引擎全部项及其他有权菜单均不得丢失。
- [ ] `R1-03` 逐页用浏览器滚轮滚到菜单底部，验证 8 页与 Buddy 的菜单项、分组、顺序和可达性一致；不得只检查首屏。
- [ ] `R1-04` 修复折叠/展开：八页都能收起、恢复、保持当前路由高亮，并在刷新/跳页后按统一策略保持状态。
- [ ] `R1-05` 统一顶部栏、面包屑、租户/工作区、搜索、日期/主操作区域，消除顶部挤压、遮挡和跨页高度漂移。
- [ ] `R1-06` 统一固定区与滚动区：底部能力带、明日预告、抽屉和浮层不得遮挡任务、数字同事或页面主内容。
- [ ] `R1-07` 统一公共按钮语义：可执行、等待条件、只读、禁止外部副作用四态可辨；移除“看似可点但只弹预检提示”的假可用态。
- [ ] `R1-08` 建立 Shell 自动回归：8 页×完整菜单×滚到底×折叠/展开×当前项高亮×三视口×键盘/焦点，共用一个断言源。
