...
> 状态：**v1.1 架构已评审冻结；v1.2 新电商任务增补待评审**
> 版本：v1.2 · 2026-08-03
...
> 客户需求增补：[新电商达人增长与控价任务产品方案](228-新电商达人增长与控价任务产品方案.md)
...
| `platform.ecommerce.wechat-shop` | 微信小店/优选联盟授权数据、达人合作 capability、平台枚举与回执 | 达人评分、邀约策略和价格治理策略 |
| `platform.ecommerce.kuaishou` | 快手小店、达人撮合及直播/商品 capability 映射 | 跨平台达人任务与通用价格规则 |
...
| 达人增长 | `CreatorDiscoveryBrief`、`CreatorCandidateSnapshot`、`CreatorScorecard`、`CreatorShortlistRevision`、`CreatorCollaborationCase`、`CreatorPerformanceReview` |
| 价格治理 | `PriceGovernancePolicyRevision`、`PriceObservationEvidence`、`ProductIdentityMatch`、`PriceViolationCandidate`、`PriceEnforcementDraft`、`PriceRestorationCheck` |
...
| G1 | 调度、只读 Research/Metric adapter 基座 | D01～D06、DailyBrief、GrowthReview、达人发现与价格监控任务模板 | 数据/达人/价格只读 capability | 时区、目标、筛选条件、价格策略与指标映射 |
| G2 | 通用 Artifact/Observation | CustomerLite、C/G/S Logic、工作台 | 商品/订单只读映射 | consent/启用范围 |
| G3 | 通用 Experiment/Job 外壳（如确需） | 私域、活动、达人合作批次、归因业务模型 | 渠道数据映射 | 预算/频控/活动/达人合作参数 |
| G4 | Memory Candidate/Item/Policy/Retrieval 基座 | EcommerceExperience 与六 Agent 策略 | 无 | scope/retention 配置 |
| G5 | Connector Runtime/OAuth/Webhook 基座 | 内容/直播业务模型与安全策略 | 六社交平台 adapter，含微信小店/抖音/快手达人 capability | 账号 secret_ref/capability snapshot |
