...
> 状态：**v1.1 · IMPLEMENTING（用户已授权全量编码）**
> 上位依据：`../04-228-AIP受控Action权限Draft与审批闭环实施方案.md`
> 对应阶段：AIP-3；前置：02 Task/Run 主链 GREEN。

## 0. 实时基线与实施子波（2026-08-11）

- 代码基线：`m1@9bf5757`；迁移唯一 head 为 `aip1_001`；正向范围只允许 `org-org/dev-project`，`dev-org` 只作负向 canary。
...
| 04-05 | 实现 maker-checker、职责分离、break-glass | approval service | 双人/时限/事故 Receipt |
...
5. 发布 `/v1/aip/action-proposals` 的 create/list/get/draft/approve/reject/timeline API；AIP-3A 不发布 execute/lease Adapter 能力。
