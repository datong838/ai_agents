...
> 第一行：**AI操作系统**  
> 第二行：**AOS 企业AI转型方案**  
> **覆盖**：工作台 → AIP 决策引擎 → 本体数字孪生 → 数据操作系统 → **交付 Apollo**  
...
3. 侧栏顺序：**概览** → **工作台** → **AIP** → **本体** → **数据集成** → **交付 Apollo**。
...
**v2.0** · 截图风格全站刷新 · Appearance：浅色 / 深色 / 跟随系统（顶栏「外观」）

### v2.0 变更
- 全新深色主题配色：`#0b0e17` 背景 + `#0d111c` 侧栏 + `#141a2b` 卡片
- 更精致的边框（`0.055` 透明度）和阴影系统
- 侧栏宽度从 224px 调整为 220px，更紧凑的导航项
- 顶部栏高度统一 52px，搜索框/外观按钮/通知/头像全部刷新
- 首页工作台区域完全按截图重排：唯一入口卡片 + 三模块卡片 + AIP 底部条
- 55 个子页面 shell 样式批量统一（保留各模块图标色差异）
