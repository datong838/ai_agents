---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/available-connectors/amazon-kinesis/",
  "title": "Amazon Kinesis",
  "page_id": "amazon-kinesis",
  "category_id": "data-integration",
  "section_id": "available-connectors",
  "previous": "/zh/foundry/available-connectors/amazon-dynamodb/",
  "next": "/zh/foundry/available-connectors/amazon-marketplace/",
  "scraped_at": "2026-07-13T05:34:42.711554+00:00"
}
---
...
| partition\_key (字符串) | data (字符串)                              | kinesis\_ingestion\_timestamp (时间戳) | foundry\_ingestion\_timestamp (时间戳)
|---                     |---                                          |---                          |---
...
Kinesis 连接器将消息内容解析为 Unicode 字符串。使用下游流变换（例如，[Pipeline Builder](/zh/foundry/pipeline-builder/transforms-overview/)中的`parse_json`）解析结构化数据。

* `partition_key` 列将包含用于将消息发布到 Kinesis 的分区键。
* `kinesis_ingestion_timestamp` 列将包含将消息发布到 Kinesis 的时间戳。
* `foundry_ingestion_timestamp` 列将包含消息被 Foundry 吸收的时间戳。
...
Kinesis 连接器保证具有相同 `partition_key` 的消息的传递顺序。具有不同 `partition_key` 值的消息可能以任何顺序处理。

## 设置

1. 打开[数据连接](/zh/foundry/data-connection/overview/)应用程序，并在屏幕右上角选择\*\*+ 新建源\*\*。
2. 从可用的连接器类型中选择**Kinesis**。
...
为您的 Kinesis 连接选择一种认证方法：[AWS 实例](#aws-instance)或[静态凭证](#static-credentials)。
...
Kinesis 连接器可以选择在连接到 Kinesis 流之前承担一个 STS 角色。有关这些参数的详细信息，请参考 [AWS 文档 ↗](https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html)。
...
了解如何在[设置流式同步](/zh/foundry/data-connection/set-up-streaming-sync/)教程中设置与 Kinesis 的同步。

## 以 Kinesis 格式导出数据

连接器支持在数据连接中以外部 Kinesis 流的方式导出。

要以 Kinesis 格式导出，首先为您的 Kinesis 连接器[启用导出](/zh/foundry/data-connection/export-overview/#enable-exports-for-source)。然后，[创建一个新的导出](/zh/foundry/data-connection/export-overview/#creating-a-new-export)。
...
| `分区列` | 是 | 第一个字符串列 | 将用于确定数据记录在流中属于哪个分片的列。必须是一个字符串值，通常是主键。查看 [AWS 文档 ↗](https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html) 获取更多信息。|
