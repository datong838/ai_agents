---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/data-health/builds-checks-faq/",
  "title": "搭建和检查常见问题解答",
  "page_id": "builds-checks-faq",
  "category_id": "data-integration",
  "section_id": "data-health",
  "previous": "/zh/foundry/data-health/overview/",
  "next": "/zh/foundry/data-health/check-types/",
  "scraped_at": "2026-07-13T06:04:08.762727+00:00"
}
---
...
有关一般信息，请查看我们的[搭建](/zh/foundry/data-integration/builds/)和[健康检查](/zh/foundry/data-health/overview/)文档。

* [如何解决一般错误？](#how-do-i-resolve-general-errors)
* [如何理解冗长或令人困惑的错误信息？](#how-do-i-understand-a-long-or-confusing-error-message)
* [为什么我的搭建“等待资源”？](#why-are-my-builds-waiting-for-resources)
* [为什么我的搭建运行时间变长了？](#why-are-my-builds-taking-longer-to-run)
* [如何解决不平衡（偏斜）的合并？](#how-do-i-resolve-imbalanced-skewed-joins)
* [错误信息中引用的“shrinkwrap”是什么意思？](#what-does-shrinkwrap-mean-when-referenced-in-an-error-message)
* [在错误信息中看到“PERMISSION\_DENIED”](#seeing-permission_denied-in-error-message)
* [如何解决“等待输入依赖计算”的错误信息？](#how-do-i-resolve-the-waiting-for-input-dependencies-to-be-computed-error-message)
* [我的CI任务失败因为仓库不拥有数据集](#my-ci-job-fails-because-repository-does-not-own-dataset)
* [为什么我的检查因超时失败？](#why-are-my-checks-failing-on-timeout)
...
1. 搭建是否曾经成功过？如果是，您是否更改了生成数据集的逻辑？尝试回滚这些更改；如果搭建成功，您可能可以将问题隔离到新逻辑。您还可以在**搭建**应用的**数据集**窗格中选择**日志**以查看搭建历史。

2. 底层数据最近是否更改了？您可能会看到以下几种表现：
   * 如果数据量急剧增加，您可能会因为计算资源问题而看到性能下降或搭建失败。
   * 底层数据的模式是否有任何变化？当存在依赖特定模式的逻辑时，这通常会导致搭建失败。要检查，请在搭建的**数据集**窗格中选择**比较**以比较与以前交易的差异。
   * 您失败的数据集上游的任何数据集的逻辑是否发生了变化？这可能会导致数据量增加或模式更改的下游影响。

[返回顶部](#builds-and-checks-faq)
...
3. 在[联系支持](/zh/foundry/getting-started/support-overview/)时，请始终包括以下内容：
   * 您收到的完整错误信息
   * 您已采取的任何故障排除步骤
   * 您遇到错误的确切时间（日期、时间、时区）
...
您的搭建卡在“等待资源”状态的时间比平时长，并且没有运行。这可能是由于您运行搭建时平台上的活动增加所致。这时运行的许多搭建可能正在使用平台上的可用资源，导致您的搭建排队等待其他搭建完成并释放资源。这种行为是[Spark变换](/zh/foundry/code-workbook/transforms-spark/)讨论的Spark执行模型的副产品。
...
* 秘诀在于`EXPLODE`函数。`EXPLODE`是一个笛卡尔积：`SELECT 'a' AS test, EXPLODE(ARRAY(1,2,3,4,5)) AS dummy2`
* 结果如下：
...
此表格展示了两列数据，`test`和`test2`，其中`test`列的值都是`a`，而`test2`列的值是从1到5的整数。
...
* 验证倾斜的另一种方法是，在Contour中打开每个表，对`something`列进行透视并聚合行数，然后在`something`上合并左右表并乘以聚合列。这将输出每个`something`键的行数，理想情况下应均匀分布，但当出现倾斜时，表明需要进行加盐合并。
* 考虑将您的代码替换为如下所示：
...
* 要查找给定仓库的 shrinkwrap 文件：
  ...
  * `transforms-shrinkwrap.yml` 文件应如下所示：
  ...
  ![transforms.shrinkwrap.yml](/resources/foundry/data-health/shrinkwrap@2x.png)
