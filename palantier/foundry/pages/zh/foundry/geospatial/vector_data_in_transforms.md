---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/geospatial/vector_data_in_transforms/",
  "title": "在变换中使用矢量数据",
  "page_id": "vector_data_in_transforms",
  "category_id": "data-integration",
  "section_id": "geospatial",
  "previous": "/zh/foundry/geospatial/raster_data/",
  "next": "/zh/foundry/geospatial/ontology/",
  "scraped_at": "2026-07-13T06:18:16.620718+00:00"
}
---
...
您可以使用Pipeline Builder应用程序加载、变换和运用地理空间数据，而无需编写任何代码。Pipeline Builder的地理空间套件几乎与下文描述的Geospatial Tools Python库相媲美。[了解有关Pipeline Builder地理空间功能的更多信息。](/zh/foundry/pipeline-builder/transforms-geospatial/)
:::
...
* [清理地理空间数据](#cleaning-geospatial-data)
* [空间函数](#spatial-functions)
* [空间合并](#spatial-joins)

## 安装

[导入](/zh/foundry/transforms-python/use-python-libraries/) `geospatial-tools` 和 [Spark 配置文件](/zh/foundry/optimizing-pipelines/apply-spark-profiles/) `GEOSPARK` 到您的Python库中。如需安装帮助，请联系您的Palantir代表。
...
1. 表格化的，因此数据可以在 Spark 变换中使用
2. 格式化为有效的 [GeoJSON ↗](https://en.wikipedia.org/wiki/GeoJSON) 或 geohash，因此数据可以在 Foundry Ontology 中使用
3. 使用 [EPSG:4326 ↗](https://epsg.io/4326) CRS 投影，以便空间合并的双方使用相同的投影，Foundry 地图将正确渲染特征。

`geospatial-tools` 库提供了 `clean_geometry()` 和 `lat_long_to_geometry()` 来执行步骤 (2) 和 (3)。如果您的原始数据是 GeoJSON、Shapefile、GDB、KML 或 KMZ，您可以使用[内置解析辅助函数](#parsing-helper-functions)来执行步骤 (1)。
...
* 参数
  ...
  * `geometry_format` (`str`, 非必填): 输入几何格式。支持的格式有 `geojson`、`geohash`（包含 `latitude,longitude` 的字符串）、`wkt` 和 `wkb`。默认是 `geojson`。
  * `lat_long` (`bool`, 非必填): 如果输入几何坐标按 `latitude,longitude` 排列，则为 `true`，这可能是 `geohash` 输入的情况，但对于 `geojson` 或 `wkt` 则不是。默认是 `false`。
  * `output_format` (`str`, 非必填): 输出几何格式（`geojson` 或 `geohash`）。默认是 `geojson`。
* 示例
  ```python
...
* 参数
  ...
  * `drop_invalid_layers` (`bool`, 非必填): 默认为 `false`。如果设置为 `true`，则会默默删除 KML 文件中空的/无法解析的层。否则，会引发出错。
* 示例
  ```python
  from transforms.api import transform, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  ...
  * `metric_crs` (`str`, 非必填): 用于转换和缓冲的指标 CRS（*注意：此投影在中间步骤中使用。输出将保持与输入相同的投影*）。应在目标区域附近居中以最小化失真。默认是 `EPSG:3395`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  * `geometry` (`str`): GeoJSON 几何列名称。
  * `segments` (`int`, 非必填): 构成圆的线段数。默认是 `100`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  ...
  * `metric_crs` (`str`, 非必填): 用于计算容差的指标 CRS（*注意：此投影在中间步骤中使用。输出将保持与输入相同的投影*）。应在目标区域附近居中以最小化失真。默认是 `EPSG:3395`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  * `geometry1` (`str`): 第一个 GeoJSON 几何列名称。
  * `geometry2` (`str`): 第二个 GeoJSON 几何列名称。
  * `metric_crs` (`str`, 非必填): 用于计算距离的指标 CRS。应在目标区域附近居中以最小化失真。默认是 `EPSG:3395`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  * `geometry` (`str`): GeoJSON 几何列名称。
  * `metric_crs` (`str`, 非必填): 用于计算长度的指标 CRS。应在目标区域附近居中以最小化失真。默认是 `EPSG:3395`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  * `geometry` (`str`): GeoJSON 几何列名称。
  * `metric_crs` (`str`, 非必填): 用于计算面积的指标 CRS。应在目标区域附近居中以最小化失真。默认是 `EPSG:3395`。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  ...
  * `geometry2` (`str`): 要针对 `geometry1` 测试的 GeoJSON 列。
* 示例
  ```python
  from transforms.api import transform_df, Input, Output
  from geospatial_tools import geospatial
...
* 参数
  ...
  * `how` (`str`, 非必填): 合并类型（支持与常规 PySpark 合并相同的合并类型）。默认是 `inner`。
  * `library` (`str`, 非必填): 使用哪个库进行合并，`h3` 或 `sedona`。默认是 `h3`。
  * `resolution` (`int`, 非必填): 用于合并的 H3 分辨率（注意：这不影响结果，仅用于优化）。如果包含 `library='sedona'`，则会引发异常。如果未包含，将推断出最佳猜测分辨率。
  * `left_partitions` (`int`, 非必填): 使用 `library=sedona` 时左数据帧默认重新分区方案的可选覆盖。如果包含时使用 H3，会引发异常。
  * `right_partitions` (`int`, 非必填): 使用 `library=sedona` 时右数据帧默认重新分区方案的可选覆盖。如果包含时使用 H3，会引发异常。
...
* 对于大型数据规模，使用上面记录的 `DataFrame.spatial_join` 函数。
* 参数
  * `df_left` ([`pandas.DataFrame` ↗](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html)): 左输入数据帧。
  * `df_right` ([`pandas.DataFrame` ↗](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html)): 右输入数据帧。
  * `geometry_left` (`str`): 左数据帧的几何列名称。
  * `geometry_right` (`str`): 右数据帧的几何列名称。
  * `how` (`str`, 非必填): 合并类型，{`left`, `right`, `inner`} 之一。默认是 `inner`。
  * `op` (`str`, 非必填): 合并条件，{`intersects`, `contains`, `within`} 之一。默认是 `intersects`。
* 示例
  ```python
  from transforms.api import transform, Input, Output
