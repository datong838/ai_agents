---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/hyperauto/configuration-options/",
  "title": "HyperAuto V2 配置选项",
  "page_id": "configuration-options",
  "category_id": "data-integration",
  "section_id": "hyperauto",
  "previous": "/zh/foundry/hyperauto/proposals/",
  "next": "/zh/foundry/hyperauto/folder-based-sap/",
  "scraped_at": "2026-07-13T05:33:38.411856+00:00"
}
---
...
有关 HyperAuto V1 配置参考，请参阅[旧版文档](/zh/foundry/hyperauto/v1-configuration-reference/)。
:::
...
![HyperAuto V2 源配置向导](../../../images/foundry/hyperauto/hyperauto-v2-wizard-2-source-config.png)

### 源系统

此选项适用于用户必须在子系统之间选择的源（例如 [SAP 中的“上下文”](#sap-source-systems)）。子系统被定义为源中的配置，导致其自身的可用表和元数据集。因此，在子系统之间切换将完全更改其他可用配置，例如支持的[管道模式](#pipeline-modes)（批处理与流式处理）和在[**输入配置**](#input-configuration)页面上可供选择的表和现有同步。

#### SAP 源系统

将 Foundry 连接到 SAP 系统有三种主要的[架构模式](/zh/foundry/sap/architecture/)：

* **直接连接：** 连接器安装在 ERP 系统本身的应用程序服务器上，提供对表的直接访问。
* **SLT：** 连接器安装在 SAP SLT 复制服务器上，该服务器连接到基础 ERP 系统。使用流式[管道模式](#pipeline-modes)需要 SLT。
* **远程：** 连接器安装在连接到基础 ERP 系统的“网关”应用程序服务器上。通常用于 SAP 源不满足连接器[先决条件](/zh/foundry/sap/overview/#prerequisites)的情况。
  在 SLT 或远程连接的情况下，用户必须选择用于识别要连接的 SAP 子系统的上下文。

### 管道模式

HyperAuto 支持两种同步和数据变换模式。您可以在**源配置**页面上的初始 HyperAuto 管道设置中选择流式处理或批处理模式。
...
![输入配置向导](../../../images/foundry/hyperauto/hyperauto-v2-wizard-3-add-add-to-cart.png)
...
:::callout{theme="neutral" title="Beta 功能"}
同步创建是一个 Beta 功能，可能在您的 Foundry 实例上不可用。如果您有兴趣使用此功能，请联系您的平台管理员。
:::

您的 Foundry 注册可能在**建议**选项卡上启用了 AIP 功能；更多信息可以在[AIP 文档](/zh/foundry/hyperauto/aip/)中找到。
...
* [配置选项](#configuration-options)
  * [自动合并](#automatic-joins)
  * [人类可读的列名称](#human-readable-column-names)
  * [生成主键](#generate-primary-keys)
  * [生成外键](#generate-foreign-keys)
  * [去重行](#deduplicate-rows)
  * [数据清理](#data-cleaning)

![管道配置向导](../../../images/foundry/hyperauto/hyperauto-v2-wizard-4-pipeline-config.png)
...
<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-auto-join.png" alt="自动合并示例" width="400" />
...
<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-rename-cols.png" alt="人类可读的列名称" width="400" />
...
<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-gen-pk.png" alt="生成主键" width="400" />
...
<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-gen-fk.png" alt="生成外键" width="400" />

HyperAuto 还可以访问源数据模型元数据中定义的对象到对象关系。使用这些元数据，可以在管道中创建逻辑，为每个关系生成外键列（通过连接相关列，类似于[主键逻辑](#generate-primary-keys)），可以用于合并或从中构建 Ontology 链接。

外键命名的形式为 `column1_column2_|_foreign_key_tableA`，其中：

* 列值通过使用分隔符 `_|_` 连接 `column1` 和 `column2` 组合而成，且
* 存在一个外部关系，使得用户可以通过此列合并此表与 `tableA` 的 `primary_key`。

外键是为对象之间创建 Ontology 关系所必需的。

当启用[自动合并](#automatic-joins)配置选项时，不会为对象到丰富表的关系创建外键。

#### 去重行

<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-dedupe.png" alt="去重行" width="400" />
...
<img src="../../foundry-docs/hyperauto/media/hyperauto-v2-pipeline-config-data-clean.png" alt="数据清理" width="400" />
