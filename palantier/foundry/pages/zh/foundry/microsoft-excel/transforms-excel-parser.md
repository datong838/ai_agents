---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/microsoft-excel/transforms-excel-parser/",
  "title": "变换 Excel 解析器",
  "page_id": "transforms-excel-parser",
  "category_id": "data-integration",
  "section_id": "microsoft-excel",
  "previous": "/zh/foundry/media-sets-advanced-formats/add-audio-transcription/",
  "next": "/zh/foundry/data-integration/foundry-s3-api/",
  "scraped_at": "2026-07-13T06:22:55.208223+00:00"
}
---
...
在 Foundry 中，有多种方式可以从包含 Microsoft Excel 文件的无模式数据集中提取表格数据，包括管道搭建器、使用开源库如 [openpyxl ↗](https://openpyxl.readthedocs.io/en/stable/tutorial.html) 的[基于 Python 文件的变换](/zh/foundry/transforms-python/unstructured-files/)和使用开源库如 [Apache POI ↗](https://poi.apache.org/) 的[基于 Java 文件的变换](/zh/foundry/transforms-java/unstructured-files/)。

除了这些选项外，Palantir 提供了一个名为 `transforms-excel-parser` 的库，它封装了 Apache POI 的合理默认行为，使其可以在 [transforms-java](/zh/foundry/transforms-java/overview/) 存储库中轻松使用，且无需复杂配置。
...
* 通过 `ParseResult` 类上的 `errorDataframe()` 方法支持快速失败和安全失败行为，可以在运行时检查以失败任务，或者将其写入单独的输出并异步检查。
...
![transforms-excel-parser 库搜索。](../../../images/foundry/microsoft-excel/transforms-excel-parser-library-search.png)

选择最新版本。

<img src="../../foundry-docs/microsoft-excel/media/transforms-excel-parser-latest-version-available.png" alt="选择 transforms-excel-parser 的最新可用版本。" width="220"/>

这将显示一个对话框，用于将 `transforms-excel-parser-bundle` 作为额外的支持存储库导入。选择“添加库”。

<img src="../../foundry-docs/microsoft-excel/media/add-transforms-excel-parser-bundle.png" alt="确认库依赖更改提示。" width="450" />

:::callout{theme="neutral"}
您可能会看到 `eddie-spark-module-bundle` 和/或 `ri.eddie.artifacts.repository.bundles` 除了 `transforms-excel-parser-bundle` 之外的下拉选项。

名称中带有 `eddie` 的支持存储库专为 [管道搭建器](/zh/foundry/pipeline-builder/overview/) 应用程序专用，因此它们不是合适的选择，使用它们可能会导致未来的问题。
...
![transforms-excel-parser gradle 依赖代码片段。](../../../images/foundry/microsoft-excel/transforms-excel-parser-gradle-dependency.png)
...
有关详细的 API 文档，[下载 javadoc 存档](../../foundry-docs/microsoft-excel/media/transforms-excel-parser-javadoc-1.155.0.zip)，解压缩并在网络浏览器中查看包含的 HTML 文件。
...
* 变换任务失败，并出现类似 `Spark module '{module_rid}' died while job '{job_rid}' was using it` 的出错。
* 变换任务长时间停滞，既未成功也未失败。

判断任务是否停滞的一个粗略指南是处理单个文件是否需要超过 10 分钟，因为在内存充足的情况下，一个非常大的 Excel 文件可能需要大约 8 分钟处理。请注意，一个 Spark 任务可以处理多个输入文件，因此应用此规则时并不总是简单直接。

无论内存不足的症状是任务失败还是任务停滞，建议通过切换到每个任务提供更多内存的 [Spark 配置文件](/zh/foundry/optimizing-pipelines/spark-profiles-reference/) 组合来解决问题，例如 `EXECUTOR_MEMORY_LARGE` 和 `EXECUTOR_CORES_EXTRA_SMALL`。
...
在此示例中，我们注册了多个`FormParser`实例，但也可以注册`FormParser`和`TableParser`实例的组合，这在包含表格元素的复杂表单中是一种常见模式（可以在同一工作表内或跨工作表）。
