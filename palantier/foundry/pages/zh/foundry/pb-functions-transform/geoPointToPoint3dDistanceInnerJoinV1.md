---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/pb-functions-transform/geoPointToPoint3dDistanceInnerJoinV1/",
  "title": "GeoPoint-to-GeoPoint 3d距离内合并",
  "page_id": "geoPointToPoint3dDistanceInnerJoinV1",
  "category_id": "data-integration",
  "section_id": "pb-functions-transform",
  "previous": "/zh/foundry/pb-functions-transform/geoIntersectionLeftJoinV1/",
  "next": "/zh/foundry/pb-functions-transform/geoIntersectionJoinV1/",
  "scraped_at": "2026-07-13T05:58:33.030945+00:00"
}
---
...
* **选择左侧列的条件** - 将测试左侧输入模式中的所有列以查看它们是否符合此条件。如果符合，该列将在输出中被选中。<br>*ColumnPredicate*
* **选择右侧列的条件** - 将测试右侧输入模式中的所有列以查看它们是否符合此条件。如果符合，该列将在输出中被选中。<br>*ColumnPredicate*
* **距离** - 在此距离内合并几何，以坐标参考系统的单位为准。<br>*Literal\<DefiniteNumeric>*
* **合并键** - 用于合并的左侧和右侧输入中的geojson列。<br>*Tuple\<Column\<Geometry>, Column\<Geometry>>*
* **左侧数据集** - 用于合并的左侧数据集。<br>*Table*
* **投影坐标系统** - 合并前输入几何将转换为此坐标系统，并将以给定坐标系统的单位测量距离。格式为"authority:id"，例如UTM第18N区可通过EPSG:32618识别。<br>*Literal\<String>*
* **右侧数据集** - 用于合并的右侧数据集。<br>*Table*
* **使用z坐标** - 是否包括z坐标并计算三维距离。如果为false，z坐标将被忽略并计算二维距离。<br>*Literal\<Boolean>*
* *非必填* **右侧列的前缀** - 在右侧所有列前添加的前缀。<br>*Literal\<String>*
...
* **选择左侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[geometryColLhs, lhs-1],<br>)
* **选择右侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[],<br>)
* **距离**: 10.0
* **合并键**: (`geometryColLhs`, `geometryCol`)
* **左侧数据集**: ri.foundry.main.dataset.left
* **投影坐标系统**: EPSG:4326
...
* **选择左侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[],<br>)
* **选择右侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[geometryCol, arrayCol],<br>)
* **距离**: 10.0
* **合并键**: (`geometryColLhs`, `geometryCol`)
* **左侧数据集**: ri.foundry.main.dataset.left
...
* **选择左侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[geometryColLhs, lhs-1],<br>)
* **选择右侧列的条件**: <br>columnNameIsIn(<br> columnNames: \[geometryCol, col1, arrayCol],<br>)
* **距离**: 10.0
* **合并键**: (`geometryColLhs`, `geometryCol`)
* **左侧数据集**: ri.foundry.main.dataset.left
* **投影坐标系统**: EPSG:4326
