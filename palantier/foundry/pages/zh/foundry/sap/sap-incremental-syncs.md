---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/sap/sap-incremental-syncs/",
  "title": "增量更新",
  "page_id": "sap-incremental-syncs",
  "category_id": "data-integration",
  "section_id": "sap",
  "previous": "/zh/foundry/sap/sap-streaming-sync-setup/",
  "next": "/zh/foundry/sap/sap-object-types/",
  "scraped_at": "2026-07-13T05:39:00.601901+00:00"
}
---
...
* **多个字段：** 类似于**单个字段**，但支持使用 OR 运算符组合多个字段；例如，当表中同时存在“创建”和“最后更新”时间戳字段时可以使用。
* **字段连接：** 类似于**多个字段**，但不是用 OR 运算符组合字段，而是将字段连接在一起；例如，当表中有独立的“最后更新日期”和“最后更新时间字段”时可以使用。
* **使用更改文档表：** 基于 SAP 的更改文档表中的更新导入新行。**警告：这是一个实验性功能，可能并不总是能捕捉到所有更改；谨慎使用。**
* **使用双表或双表合并：** 当“双表”中的字段大于或等于已导入的最大值时，从目标表中导入新行。**双表合并**允许将目标表设置为多个表的合并。
...
同步定义中的**增量字段**属性用于管理增量更新。在初始批量提取后，每次增量更新仅提取已更改的行。使用**多个字段**类型时，字段列表应以逗号分隔。

:::callout{theme="neutral"}
要启用增量更新功能，请将**事务类型**设置为**追加**。
:::
...
有关 SLT 增量更新的更多详情，请参阅[配置 SLT 的文档](/zh/foundry/sap/configure-sap-slt/)。
...
![双表增量](../../../images/foundry/sap/twin_incremental.png)

在此示例中，成功的初始加载后，后续请求将仅从 VBAP 中提取数据行，其中 VBAK（头表）中的 UPD\_TMSTMP 字段大于或等于上次成功同步中的最高 UPD\_TMSTMP 值。

**增量双表**设置用于指明要使用的双表。

**增量双映射**设置用于定义主表和双表之间的合并条件。语法如下：
