---
{
  "source_url": "https://www.palantir.com/docs/zh/foundry/transforms-python/pyspark-logging/",
  "title": "日志记录",
  "page_id": "pyspark-logging",
  "category_id": "data-integration",
  "section_id": "transforms-python",
  "previous": "/zh/foundry/transforms-python/pyspark-window/",
  "next": "/zh/foundry/transforms-python/pyspark-other/",
  "scraped_at": "2026-07-13T06:08:25.022532+00:00"
}
---
...
Python 内置的 `print` 将输出到代码编辑器右侧 [代码工作簿](/zh/foundry/code-workbook/overview/) 的输出部分，错误通常会显示在那里。
...
代码存储库使用 Python 内置的[日志库 ↗](https://docs.python.org/3/library/logging.html)。这在网上有广泛的文档支持，并允许您控制日志级别（`ERROR`，`WARNING`，`INFO`），以便于筛选。

日志输出会出现在您的输出数据集的日志文件中，以及您的搭建的驱动程序日志中（`Dataset -> Details -> Files -> Log Files`，以及`Builds -> Build -> 任务状态日志`; 分别选择`"Driver logs"`）。
...
* `INFO` 表示日志级别为信息级别，通常用于记录程序正常运行的信息。
* `[2018-01-01T12:00:00]` 表示日志记录的时间戳。
* `some_transformation` 可能是执行的某个转换或操作的名称。
* `example log output` 是日志的具体输出信息。

### 从Python UDF内部记录日志

Spark会捕获从创建查询的顶级驱动程序进程输出的日志，例如上面的`some_transformation`函数。然而，它不会捕获从[用户自定义函数（UDFs）](/zh/foundry/transforms-python/pyspark-udfs/)内部写入的日志。如果您在PySpark查询中使用UDF并需要记录数据，请创建并调用第二个UDF，返回您希望捕获的数据。
...
注意：在 `withColumn` 的括号中，缺少了右括号 `)`，我已在代码中补充。

## 示例

我们经常想记录查询中的信息。PySpark 有多种方式检查 DataFrame，我们可以将这些信息发送到上述的日志记录机制。

在这些示例中，我们将使用 Code Workbook 的`print`语法，但`print`可以替换为 Transforms & Authoring 的`logger`。

### DataFrame 列

我们可以使用`df.columns`检查 DataFrame 上存在的列。这会生成一个字符串列表。
...
您可以通过调用 `.explain()` 来访问Spark将用于生成给定`DataFrame`的优化物理计划。
...
* 其次，过滤掉`employee_id`为空的记录，读取CSV格式的数据文件。
* 最终，使用`employee_id`进行连接，并投影出所需的字段：`employee_id`、`name`、`birthday`和`phone_number`。
...
将过多数据导入您的Python环境可能会导致内存耗尽。只 `collect()` 少量数据。
:::
