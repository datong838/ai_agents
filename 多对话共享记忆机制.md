...
本次先完成两项低风险基础工作：固化 v1.1 方案，以及把 Prime Agent 启动改为不携带 Key。`project_revision`、投影 manifest、`memory-status` 和 `memory-sync` 作为后续独立实施波次，不在本次安全整改中隐式上线。
...
`write_lease` 不是操作系统锁，而是协作约定。它可以先从人工台账开始，后续再实现自动冲突检测。
...
`~/.zshrc` 不再承担 Prime Agent 凭据注入责任。即使为了兼容其他工具暂时保留旧配置，安全启动器也必须清除继承变量；完成 Key 轮换后应彻底移除旧导出行。
...
| 阶段 | 状态 | 当前裁决 |
|---|---|---|
| Phase 0 | `IN_PROGRESS` | 已具备稳定工程基线和 Prime 项目投影，尚未分配首个真实 `project_revision` |
| Phase 1 | `PLAN_APPROVED` | 协议已评审，manifest、元数据和 Receipt 模板待实施 |
| Phase 2 | `PLAN_APPROVED` | 先做只读状态命令 |
| Phase 3 | `PLANNED` | 必须在 Phase 2 验收后实施 |
| Phase 4 | `PLANNED` | 不提前绑定现有交付门 |
| Phase 5 | `PLANNED` | 并行规模达到阈值后再做 |
...
最合适的实施顺序是：先完成本次无 Key 启动整改并封板 v1.1；随后由掌握最新工程现场的 AOS 主开发对话完成 Phase 0 真实 revision 基线和 Phase 1 manifest/Receipt 协议；再独立实施 Phase 2 只读漂移检查。只有 Phase 2 能稳定识别 STALE/DRIFTED/UNAVAILABLE 后，才进入自动写投影的 Phase 3。
