...
Prime Agent 投影是特殊的强一致适配器：不得把“找到 global Harness 条目、保留历史、递增版本”交给模型自由执行。实现必须对 `harness_state.json` 做确定性、受锁、原子、可回滚的结构化更新，并额外维护最后成功的条目版本及内容哈希；Prime 原生 Harness CRUD 与 AOS 同步适配器必须共用同一把文件锁，陈旧进程不得覆盖新快照。若条目版本低于已记录基线，即使 revision/hash marker 正确也必须判定为 `DRIFTED`；恢复时从最后成功版本单调递增，不能从错误的 `v1` 继续。同一 authority revision 重复同步必须幂等。
